...
from coffice.osv import expression
from coffice.tools.float_utils import float_round as round, float_compare
from coffice.tools import DEFAULT_SERVER_DATETIME_FORMAT
from coffice.exceptions import UserError, ValidationError
from coffice import api, fields, models, _, tools
from coffice.tests.common import Form
...
class AccountJournal(models.Model):
    ...
    invoice_reference_model = fields.Selection(string='Communication Standard', required=True, selection=[('coffice', 'Coffice'),('euro', 'European')], default='coffice', help="You can choose different models for each type of reference. The default one is the Coffice reference.")
    ...
    inbound_payment_method_ids = fields.Many2many('account.payment.method', 'account_journal_inbound_payment_method_rel', 'journal_id', 'inbound_payment_method',
        ...
        help="Manual: Get paid by cash, check or any other method outside of Coffice.\n"\
             "Electronic: Get paid automatically through a payment acquirer by requesting a transaction on a card saved by the customer when buying or subscribing online (payment token).\n"\
             "Batch Deposit: Encase several customer checks at once by generating a batch deposit to submit to your bank. When encoding the bank statement in Coffice,you are suggested to reconcile the transaction with the batch deposit. Enable this option from the settings.")
    outbound_payment_method_ids = fields.Many2many('account.payment.method', 'account_journal_outbound_payment_method_rel', 'journal_id', 'outbound_payment_method',
        ...
        help="Manual:Pay bill by cash or any other method outside of Coffice.\n"\
             "Check:Pay bill by check and print it from Coffice.\n"\
