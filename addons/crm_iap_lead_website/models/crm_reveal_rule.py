...
from coffice.addons.iap import jsonrpc
from coffice.addons.crm.models import crm_stage
from coffice.exceptions import ValidationError

_logger = logging.getLogger(__name__)

DEFAULT_ENDPOINT = 'https://iap-services.coffice.com'
...
class CRMRevealRule(models.Model):
    ...
    industry_tag_ids = fields.Many2many('crm.iap.lead.industry', string='Industries', help='Leave empty to always match. Coffice will not create lead if no match')
    ...
    def _assert_geoip(self):
        if not coffice._geoip_resolver:
    ...
    # Methods responsible for format response data in to valid coffice lead data
