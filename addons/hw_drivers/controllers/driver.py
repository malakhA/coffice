...
from coffice import http, _
from coffice.modules.module import get_resource_path
from coffice.addons.hw_drivers.tools import helpers
...
class StatusController(http.Controller):
    ...
    def connect_box(self, token):
        """
        This route is called when we want that a IoT Box will be connected to a COffice DB
        token is a base 64 encoded string and have 2 argument separate by |
        1 - url of coffice DB
        2 - token. This token will be compared to the token of COffice. He have 1 hour lifetime
        """
        server = helpers.get_coffice_server_url()
...
class Manager(Thread):
    ...
    def load_drivers(self):
        """
        This method loads local files: 'coffice/addons/hw_drivers/drivers'
    ...
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to COffice database
        """
        server = helpers.get_coffice_server_url()
        if server:
            subject = helpers.read_file_first_line('coffice-subject.conf')
            if subject:
            ...
            _logger.warning('COffice server not set')
    ...
    def run(self):
        """
        Thread that will check connected/disconnected device, load drivers if needed and contact the coffice server with the updates
        """
...
mpdm = MPDManager()
terminal_id = helpers.read_file_first_line('coffice-six-payment-terminal.conf')
if terminal_id:
    try:
