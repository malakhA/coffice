...
from coffice.addons.hw_proxy.controllers.main import drivers as old_drivers
from coffice.addons.hw_drivers.tools import helpers
from coffice.addons.hw_drivers.controllers.driver import event_manager, Driver, iot_devices
...
class KeyboardUSBDriver(Driver):
        ...
        Args:
            new_layout (dict): A dict containing two keys:
                ...
                - variant (str): An optional key to represent the variant of the
                                 selected layout
        """
        file_path = Path.home() / 'coffice-keyboard-layouts.conf'
        if file_path.exists():
            data = json.loads(file_path.read_text())
        else:
            data = {}
        data[self.device_identifier] = layout
        helpers.write_file('coffice-keyboard-layouts.conf', json.dumps(data))
    ...
    def _keyboard_input(self, scancode):
        ...
        pressed key represented by its scancode to the connected COffice instance.
