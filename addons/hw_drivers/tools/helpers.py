...
def add_credential(db_uuid, enterprise_code):
    write_file('coffice-db-uuid.conf', db_uuid)
    write_file('coffice-enterprise-code.conf', enterprise_code)
    ...
    if server:
        ...
        if path.exists():
            with path.open('r') as f:
                ...
                if cn == 'COfficeTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected COffice DB and
    ...
    if server:
        ...
        try:
            ...
            if response.status == 200:
                git = ['git', '--work-tree=/home/pi/coffice/', '--git-dir=/home/pi/coffice/.git']
                ...
                if db_branch != local_branch:
                    ...
                    os.system('/home/pi/coffice/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh')
...
def get_coffice_server_url():
    return read_file_first_line('coffice-remote-server.conf')
...
def load_certificate():
    """
    Send a request to COffice with customer db_uuid and enterprise_code to get a true certificate
    """
    db_uuid = read_file_first_line('coffice-db-uuid.conf')
    enterprise_code = read_file_first_line('coffice-enterprise-code.conf')
    if db_uuid and enterprise_code:
        url = 'https://www.coffice.com/coffice-enterprise/iot/x509'
        data = {
            'params': {
                'db_uuid': db_uuid,
        ...
        if result:
            write_file('coffice-subject.conf', result['subject_cn'])
            subprocess.check_call(["sudo", "mount", "-o", "remount,rw", "/"])
