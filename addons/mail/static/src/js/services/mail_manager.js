coffice.define('mail.Manager', function (require) {
"use strict";
...
var MailManager =  AbstractService.extend({
    ...
    _isReady: false,
    _COFFICEBOT_ID: ["COFFICEBOT", "COFFICEBOT"], // authorID for transient messages
    ...
    /**
     * Get the CofficeBot ID, which is the default authorID for transient messages
     *
     * @returns {Array<string>}
     */
    getCofficebotID: function () {
        return this._COFFICEBOT_ID;
    },
    /**
                ...
                if (
                    ...
                    if (thread.isTwoUserThread() && options.showNotification) {
                        ...
                        prom.then(function () {
                            ...
                            if (!self.call('bus_service', 'isCofficeFocused') || !query.isVisible) {
    ...
    _initializeInternalState: function () {
        ...
        // # of message received when coffice is out of focus
    ...
    _notifyIncomingMessage: function (message) {
        if (this.call('bus_service', 'isCofficeFocused')) {
            // no need to notify
            return;
        }
