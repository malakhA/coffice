...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        The logic will only be applied if cofficebot is in a chat with a user or
        if someone pinged cofficebot.

         :param record: the mail_thread (or mail_channel) where the user
            message was posted/cofficebot will answer.
        ...
        if len(record) != 1 or values.get("author_id") == cofficebot_id:
            return
        if self._is_bot_pinged(values) or self._is_bot_in_private_channel(record):
            ...
            if answer:
                ...
                record.with_context(mail_create_nosubscribe=True).sudo().message_post(body=answer, author_id=cofficebot_id, message_type=message_type, subtype_id=subtype_id)
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        if self._is_bot_in_private_channel(record):
            # main flow
            if cofficebot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
            ...
            elif cofficebot_state == 'onboarding_attachement' and values.get("attachment_ids"):
                ...
                return _("Wow you are a natural!<br/>Ping someone to grab its attention with @nameoftheuser. <b>Try to ping me using @COfficeBot</b> in a sentence.")
            elif cofficebot_state == 'onboarding_ping' and self._is_bot_pinged(values):
                self.env.user.cofficebot_state = "idle"
                return _("Yep, I am here! 🎉 <br/>You finished the tour, you can <b>close this chat window</b>. Enjoy discovering COffice.")
            elif cofficebot_state == "idle" and (_('start the tour') in body.lower()):
                self.env.user.cofficebot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif cofficebot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            elif cofficebot_state == "idle" and (('help' in body) or _('help') in body):
                return _("I'm just a bot... :( You can check <a href=\"https://www.coffice.com/page/docs\">our documentation</a>) for more information!")
                ...
                if cofficebot_state == 'onboarding_emoji':
                    ...
                    return _("Sorry, I am not listening. To get someone's attention, <b>ping him</b>. Write \"@cofficebot\" and select me.")
            ...
            return random.choice([_("Yep, COfficeBot is in the place!"), _("Pong.")])
        return False
    ...
    def _is_bot_pinged(self, values):
        cofficebot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        return cofficebot_id in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        cofficebot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        if record._name == 'mail.channel' and record.channel_type == 'chat':
            return cofficebot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
