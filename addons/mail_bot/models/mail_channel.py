...
from coffice import api, models, _
...
class Channel(models.Model):
    ...
    @api.model
    def init_cofficebot(self):
        if self.env.user.cofficebot_state == 'not_initialized':
            ...
            channel = self.with_context(mail_create_nosubscribe=True).create({
                'channel_partner_ids': [(4, partner.id), (4, cofficebot_id)],
                'public': 'private',
                'channel_type': 'chat',
                'email_send': False,
                'name': 'COfficeBot'
            })
            message = _("Hello,<br/>COffice's chat helps employees collaborate efficiently. I'm here to help you discover its features.<br/><b>Try to send me an emoji :)</b>")
            channel.sudo().message_post(body=message, author_id=cofficebot_id, message_type="comment", subtype="mail.mt_comment")
