coffice.define('mail_bot.systray.MessagingMenu', function (require) {
"use strict";
...
return MessagingMenu.include({
     ...
     * messaging menu (e.g. 'CofficeBot has a request')
     *
     * @override
     * @private
     ...
     * CofficeBot, and the CofficeBot influences the counter by 1 when it requests
    ...
    _handleResponseNotificationPermission: function (value) {
        ...
        if (value !== 'granted') {
            this.call('bus_service', 'sendNotification', _t('Permission denied'),
                _t('Coffice will not have the permission to send native notifications on this device.'));
        }
        this._updateCounter();
    },
    /**
    ...
    /**
     * Override so that it handles preview related to CofficeBot
     *
     * @override
     * @private
     * @param {MouseEvent} ev
