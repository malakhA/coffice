coffice.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    _dropPreviousCoffice: new concurrency.DropPrevious(),
    ...
    _autocomplete: function (value) {
        ...
        var cofficeSuggestions = [];
        var clearbitSuggestions = [];
        return new Promise(function (resolve, reject) {
            var cofficePromise = self._getCofficeSuggestions(value, isVAT).then(function (suggestions){
                cofficeSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                // Add Clearbit result with Coffice result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = cofficeSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            cofficeSuggestions.push(suggestion);
                        }
                    });
                }

                cofficeSuggestions = _.filter(cofficeSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(cofficeSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(cofficeSuggestions);
            };

            self._whenAll([cofficePromise, clearbitPromise]).then(concatResults, concatResults);
        });
    ...
    /**
     * Use Coffice Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
    ...
    _getCofficeSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousCoffice.add(def);
    },
    /**
