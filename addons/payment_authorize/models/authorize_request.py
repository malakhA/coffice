...
from coffice.addons.payment.models.payment_acquirer import _partner_split_name
...
class AuthorizeAPI():
    ...
    def create_customer_profile(self, partner, opaqueData):
        ...
        a corresponding payment profile to it. Note that a single partner in the Coffice
        ...
        values = {
            'createCustomerProfileRequest': {
                ...
                'profile': {
                    'description': ('COFFICE-%s-%s' % (partner.id, uuid4().hex[:8]))[:20],
    ...
    def create_customer_profile_from_tx(self, partner, transaction_id):
        ...
        values = {
            'createCustomerProfileFromTransactionRequest': {
                ...
                'customer': {
                    'merchantCustomerId': ('COFFICE-%s-%s' % (partner.id, uuid4().hex[:8]))[:20],
                    'email': partner.email or ''
                }
            }
        }
