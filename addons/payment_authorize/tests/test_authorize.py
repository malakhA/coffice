...
from coffice.addons.payment.tests.common import PaymentAcquirerCommon
from coffice.addons.payment_authorize.controllers.main import AuthorizeController
...
@coffice.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('coffice.addons.payment_authorize.models.payment', 'ValidationError')
...
@coffice.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeS2s(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on coffice/coffice + create sandbox account
        authorize.write({
