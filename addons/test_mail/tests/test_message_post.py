...
from coffice.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE_PLAINTEXT
from coffice.addons.test_mail.models.test_mail_models import MailTestSimple
from coffice.exceptions import AccessError
...
class TestMessagePost(BaseFunctionalTest, TestRecipients, MockEmails):
    ...
    def test_post_inactive_follower(self):
        # In some case cofficebot is follower of a record.
        # Even if it shouldn't be the case, we want to be sure that cofficebot is not notified
        (self.user_employee | self.user_admin).write({'notification_type': 'inbox'})
    ...
    @mute_logger('coffice.addons.mail.models.mail_mail', 'coffice.addons.mail.models.mail_thread')
