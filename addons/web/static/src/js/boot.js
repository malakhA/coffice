...
(function () {
    ...
    coffice.testing = typeof QUnit === 'object';
    coffice.remainingJobs = jobs;
    coffice.__DEBUG__ = {
    ...
    coffice.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__coffice_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: coffice.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = coffice.__DEBUG__.getDependencies(job.name);
            ...
            missing = coffice.__DEBUG__.getMissingJobs();
            failed = coffice.__DEBUG__.getFailedJobs();
            ...
            if (coffice.debug || failed.length || unloaded.length) {
                ...
                if (coffice.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        coffice.__DEBUG__.jsModules = {
            missing: missing,
    ...
    coffice.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            coffice.processJobs(jobs, services);
