coffice.define('web.CrashManager', function (require) {
"use strict";
...
var CrashManager = AbstractService.extend({
    ...
    show_error: function (error) {
        ...
        var dialog = new ErrorDialog(this, {
            title: _.str.capitalize(error.type) || _t("Coffice Error"),
        }, {
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("Coffice Warning"),
            buttons: [
...
core.crash_registry.add('coffice.exceptions.RedirectWarning', RedirectWarningHandler);

function session_expired(cm) {
    return {
        display: function () {
            cm.show_warning({type: _t("Coffice Session Expired"), message: _t("Your Coffice session expired. Please refresh the current web page.")});
        }
    };
}
core.crash_registry.add('coffice.http.SessionExpiredException', session_expired);
