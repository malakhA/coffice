...
from coffice.exceptions import UserError, AccessError
...
from coffice.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of Coffice so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at Coffice start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Coffice with at least two workers to print a pdf version of the reports.')
