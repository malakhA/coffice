...
import coffice
from coffice import api, fields, models, modules, tools, _
from coffice.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from coffice.exceptions import AccessDenied, UserError
from coffice.osv import expression
from coffice.tools.parse_version import parse_version
from coffice.tools.misc import topological_sort
    ...
    def install_from_urls(self, urls):
        ...
        try:
            ...
            if urls.get(OPENERP):
                ...
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'coffice' subdirectory ?
                ...
                _logger.info('Copy downloaded module `coffice` to `%s`', server_dir)
            ...
            if installed or to_install:
                ...
                self._cr.commit()
                coffice.service.server.restart()
                return {
                    'type': 'ir.actions.client',
                    'tag': 'home',
                    'params': {'wait': True},
