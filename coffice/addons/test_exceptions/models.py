...
import coffice.exceptions
import coffice.osv.osv
...
class m(models.Model):
    ...
    def generate_except_osv(self):
        ...
        raise coffice.osv.osv.except_osv('title', 'description')

    def generate_except_orm(self):
        # title is ignored in the new (6.1) exceptions
        raise coffice.exceptions.except_orm('title', 'description')

    def generate_warning(self):
        raise coffice.exceptions.Warning('description')

    def generate_redirect_warning(self):
        action = self.env.ref('test_exceptions.action_test_exceptions')
        raise coffice.exceptions.RedirectWarning('description', action.id, 'Go to the redirection')

    def generate_access_denied(self):
        raise coffice.exceptions.AccessDenied()

    def generate_access_error(self):
        raise coffice.exceptions.AccessError('description')
    ...
    def generate_user_error(self):
        raise coffice.exceptions.UserError('description')

    def generate_missing_error(self):
        raise coffice.exceptions.MissingError('description')

    def generate_validation_error(self):
        raise coffice.exceptions.ValidationError('description')
