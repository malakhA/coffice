...
__author__ = coffice.release.author
__version__ = coffice.release.version

# Also use the `coffice` logger for the main script.
_logger = logging.getLogger('coffice')
...
def report_configuration():
    ...
    """
    config = coffice.tools.config
    _logger.info("COffice version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', coffice.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = coffice.tools.config
    if not coffice.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = coffice.modules.registry.Registry.new(dbname)
        with coffice.api.Environment.manage():
            with registry.cursor() as cr:
                coffice.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with coffice.api.Environment.manage():
        with registry.cursor() as cr:
            coffice.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    coffice.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                coffice.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except coffice.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        coffice.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = coffice.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the coffice server (default command)"""
    def run(self, args):
        main(args)
