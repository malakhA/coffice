...
class Shell(Command):
    """Start coffice in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        coffice.cli.server.report_configuration()
        coffice.service.server.start(preload=[], stop=True)
        ...
        with coffice.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = coffice.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = coffice.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
