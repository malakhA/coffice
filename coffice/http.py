...
# *broken*
NO_POSTMORTEM = (coffice.exceptions.except_orm,
                 coffice.exceptions.AccessDenied,
                 coffice.exceptions.Warning,
                 coffice.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                coffice.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = coffice.service.common.dispatch
        elif service_name == 'db':
            dispatch = coffice.service.db.dispatch
        elif service_name == 'object':
            dispatch = coffice.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                coffice.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                coffice.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except coffice.exceptions.DeferredException as e:
        _logger.exception(coffice.tools.exception_to_unicode(e))
        coffice.tools.debugger.post_mortem(coffice.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(coffice.tools.exception_to_unicode(e))
        coffice.tools.debugger.post_mortem(coffice.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all COffice Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in coffice.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~coffice.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~coffice.api.Environment` bound to current request. """
        if self._env is None:
            self._env = coffice.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            coffice.tools.debugger.post_mortem(
                coffice.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return coffice.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        COffice implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <coffice.http.WebRequest.csrf_token`, the
          :data:`~coffice.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          COffice) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (coffice.exceptions.Warning, coffice.exceptions.except_orm,
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "COffice Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "COffice Session Expired"
...
def serialize_exception(e):
    ...
    if isinstance(e, coffice.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, coffice.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, coffice.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, coffice.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, coffice.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, coffice.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, coffice.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, coffice.exceptions.except_orm):
...
COffice URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.coffice.com/documentation/13.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through COffice via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["coffice", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('coffice.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(werkzeug.contrib.sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            uid = coffice.registry(db)['res.users'].authenticate(db, login, password, env)
        else:
    ...
    def check_security(self):
        ...
        env = coffice.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or coffice.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = coffice.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in coffice.http._generate_routing_rules([''] + coffice.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in coffice.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        coffice.registry(db).check_signaling()
                        with coffice.tools.mute_logger('coffice.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = coffice.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if coffice.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = coffice.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif coffice.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, COffice will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in coffice.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = coffice.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = coffice.tools.ustr(filename)
