...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (OpenERP) module name can be in the ``coffice.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``coffice.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['coffice', 'addons']:
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """Base class for COffice models.

    COffice models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~coffice.models.BaseModel._register` attribute may be set to False.
    """

    _auto = False
    """Whether a database table should be created (default: ``True``).
    If set to ``False``, override :meth:`~coffice.models.BaseModel.init`
    to create the database table.

    .. tip:: To create a model without any table, inherit
            from :class:`~coffice.models.AbstractModel`.
    """
    ...
    .. warning::
      ...
      :attr:`~coffice.models.Model._inherits`-ed models, the inherited field will
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = coffice.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or coffice.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from coffice.fields import Char, Integer
...
%(fields_list)s""") % {
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~coffice.addons.base.models.res_company`).

        For :class:`~coffice.addons.base.models.res_users` relational fields,
...
""")
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~coffice.fields.Integer`,
          :class:`~coffice.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~coffice.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~coffice.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~coffice.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~coffice.fields.Date` and
              :class:`~coffice.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`coffice.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`coffice.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          :class:`~coffice.fields.One2many` and
          :class:`~coffice.fields.Many2many` use a special "commands" format to
    ...
    def ensure_one(self):
        ...
        :raise coffice.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~coffice.api.Environment`
        ...
        for field, value in sorted(field_values, key=is_monetary):
            ...
            if field.relational:
                ...
                for invf in self._field_inverses[field]:
                    ...
                    # /home/dle/src/coffice/master-nochange-fp/coffice/addons/test_new_api/tests/test_new_fields.py
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted COffice models.

    COffice models are created by inheriting from this class::
