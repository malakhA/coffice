...
import coffice
import coffice.modules.db
import coffice.modules.graph
import coffice.modules.migration
import coffice.modules.registry
import coffice.tools as tools

from coffice import api, SUPERUSER_ID
from coffice.modules.module import adapt_version, initialize_sys_path, load_openerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('coffice.tests')
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    def load_test(idref, mode):
        ...
        try:
            ...
            coffice.registry(cr.dbname).clear_caches()
    ...
    migrations = coffice.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        if new_install:
            py_module = sys.modules['coffice.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = coffice.tools.config["overwrite_existing_translations"]
            ...
            if tools.config.options['test_enable']:
                ...
                report.record_result(coffice.modules.module.run_unit_tests(module_name))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, coffice.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        if not coffice.modules.db.is_initialized(cr):
        ...
        # coffice.modules.registry.Registry.new().
        registry = coffice.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['coffice.addons.%s' % (pkg.name,)]
                ...
                registry = coffice.modules.registry.Registry.new(
