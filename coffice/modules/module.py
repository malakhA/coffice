...
import coffice
import coffice.tools as tools
import coffice.release as release
from coffice import SUPERUSER_ID, api
from coffice.tools import pycompat
...
# ad_paths is a deprecated alias, please use coffice.addons.__path__
@tools.lazy
def ad_paths():
    _logger.warning('"coffice.modules.module.ad_paths" is a deprecated '
                    'proxy to "coffice.addons.__path__". Please consider '
                    'using the latter as the former is going to be '
                    'removed in the next version.',
                    exc_info=DeprecationWarning(), stack_info=True)
    return coffice.addons.__path__
hooked = False
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            _logger.warning('"openerp.addons" is a deprecated alias to '
                            '"coffice.addons". Please consider using the '
    ...
    def load_module(self, name):
        ...
        coffice_name = re.sub(r'^openerp.addons.(\w+)$', r'coffice.addons.\g<1>', name)

        coffice_module = sys.modules.get(coffice_name)
        if not coffice_module:
            coffice_module = importlib.import_module(coffice_name)

        sys.modules[name] = coffice_module

        return coffice_module
...
class COfficeHook(object):
    """ Makes coffice package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            _logger.warning('openerp is a deprecated alias to coffice. '
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'coffice\g<1>', name)
...
def initialize_sys_path():
    ...
    ``import coffice.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global hooked

    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in coffice.addons.__path__:
        coffice.addons.__path__.append(dd)

    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in coffice.addons.__path__:
            coffice.addons.__path__.append(ad)
    ...
    if base_path not in coffice.addons.__path__ and os.path.isdir(base_path):
        coffice.addons.__path__.append(base_path)

    if not hooked:
        sys.meta_path.insert(0, COfficeHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    initialize_sys_path()
    for adp in coffice.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = coffice.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in coffice.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('coffice.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['coffice.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    initialize_sys_path()
    for ad in coffice.addons.__path__:
...
class COfficeTestResult(unittest.result.TestResult):
    """
    ...
    def getErrorCallerInfo(self, error, test):
        ...
        # only test case should be executed in coffice, this is only a safe guard
...
def run_unit_tests(module_name, position='at_install'):
    ...
    """
    global current_test
    from coffice.tests.common import TagsSelector # Avoid import loop
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            result = COfficeTestRunner().run(suite)
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, coffice.sql_db.sql_counter - t0_sql)
