...
from coffice.sql_db import TestCursor
from coffice.tools import (assertion_report, config, existing_tables, ignore,
                        lazy_classproperty, lazy_property, OrderedSet)
from coffice.tools.lru import LRU
...
class Registry(Mapping):
    ...
    def __new__(cls, db_name):
        ...
        with cls._lock:
            try:
                ...
                # coffice.service.wsgi_server.application
        ...
        with cls._lock:
            with coffice.api.Environment.manage():
                ...
                try:
                    ...
                    try:
                        coffice.modules.load_modules(registry._db, force_demo, status, update_module)
                    except Exception:
                        coffice.modules.reset_modules_state(db_name)
                        raise
                except Exception:
    ...
    def init(self, db_name):
        ...
        self._db = coffice.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            has_unaccent = coffice.modules.db.has_unaccent(cr)
            if coffice.tools.config['unaccent'] and not has_unaccent:
                _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
            self.has_unaccent = coffice.tools.config['unaccent'] and has_unaccent
    ...
    def init_models(self, cr, model_names, context):
        ...
        env = coffice.api.Environment(cr, SUPERUSER_ID, context)
