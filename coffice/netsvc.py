...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('coffice.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
DEFAULT_LOG_CONFIGURATION = [
    'coffice.http.rpc.request:INFO',
    'coffice.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['coffice:DEBUG', 'coffice.sql_db:INFO', 'coffice.http.rpc:DEBUG'],
    'debug_rpc': ['coffice:DEBUG', 'coffice.sql_db:INFO', 'coffice.http.rpc.request:DEBUG'],
    'debug': ['coffice:DEBUG', 'coffice.sql_db:INFO'],
    'debug_sql': ['coffice.sql_db:DEBUG'],
    'info': [],
    'warn': ['coffice:WARNING', 'werkzeug:WARNING'],
    'error': ['coffice:ERROR', 'werkzeug:ERROR'],
    'critical': ['coffice:CRITICAL', 'werkzeug:CRITICAL'],
}
