...
import coffice
from coffice import SUPERUSER_ID
from coffice.exceptions import AccessDenied
import coffice.release
import coffice.sql_db
import coffice.tools
from coffice.sql_db import db_connect
from coffice.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not coffice.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and coffice.tools.config.verify_admin_password(passwd):
        return True
    raise coffice.exceptions.AccessDenied()

# This should be moved to coffice.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = coffice.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            coffice.modules.db.initialize(cr)
            coffice.tools.config['load_language'] = lang
            cr.commit()

        registry = coffice.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = coffice.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = coffice.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = coffice.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = coffice.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    coffice.sql_db.close_db(db_original_name)
    db = coffice.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = coffice.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = coffice.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = coffice.tools.config.filestore(db_original_name)
    to_fs = coffice.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    coffice.modules.registry.Registry.delete(db_name)
    coffice.sql_db.close_db(db_name)
    ...
    fs = coffice.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'coffice_dump': '1',
        'db_name': cr.dbname,
        'version': coffice.release.version,
        'version_info': coffice.release.version_info,
        'major_version': coffice.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with coffice.tools.osutil.tempdir() as dump_dir:
            filestore = coffice.tools.config.filestore(db_name)
            ...
            coffice.tools.exec_pg_command(*cmd)
            if stream:
                coffice.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                coffice.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = coffice.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with coffice.tools.osutil.tempdir() as dump_dir:
        ...
        if coffice.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = coffice.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = coffice.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
            ...
            if coffice.tools.config['unaccent']:
                try:
...
def exp_rename(old_name, new_name):
    coffice.modules.registry.Registry.delete(old_name)
    coffice.sql_db.close_db(old_name)
    ...
    old_fs = coffice.tools.config.filestore(old_name)
    new_fs = coffice.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    coffice.tools.config.set_admin_password(new_password)
    coffice.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        coffice.tools.config['update']['base'] = True
        coffice.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@coffice.tools.mute_logger('coffice.sql_db')
...
def list_dbs(force=False):
    if not coffice.tools.config['list_db'] and not force:
        raise coffice.exceptions.AccessDenied()

    if not coffice.tools.config['dbfilter'] and coffice.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, COffice will not
        ...
        res = sorted(db.strip() for db in coffice.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [coffice.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of COffice
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if coffice.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        coffice.sql_db.close_db(database_name)
...
def exp_list_lang():
    return coffice.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(coffice.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return coffice.release.version
