...
import coffice
from coffice.exceptions import UserError, ValidationError, QWebException
from coffice.models import check_method_name
from coffice.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = coffice.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(coffice.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if coffice.registry(dbname)._init and not coffice.tools.config['test_enable']:
                    raise coffice.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    coffice.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = coffice.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    return coffice.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with coffice.registry(db).cursor() as cr:
