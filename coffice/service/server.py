...
import coffice
from coffice.modules import get_modules
from coffice.modules.module import run_unit_tests, get_test_modules
from coffice.modules.registry import Registry
from coffice.release import nt_service_name
from coffice.tools import config
from coffice.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'coffice.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    # is included in all Python 3 releases installed on all operating systems supported by COffice.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(coffice, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in coffice.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="coffice.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from coffice.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = coffice.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(coffice.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="coffice.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="coffice.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(coffice, 'phoenix', None):
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('coffice: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(coffice.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = coffice.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('coffice.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            from coffice.addons import base
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(coffice.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            coffice.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if coffice.tools.osutil.is_running_as_nt_service():
        ...
        for mod in [m for m in get_modules() if '/%s/' % m in test_file]:
            for mod_mod in get_test_modules(mod):
                ...
                if test_path == mod_path:
                    ...
                    result = coffice.modules.module.COfficeTestRunner().run(suite)
        ...
        try:
            ...
            if config['test_file']:
                ...
                if not os.path.isfile(test_file):
                    ...
                    with coffice.api.Environment.manage():
                        load_test_file_py(registry, test_file)

            # run post-install tests
            if config['test_enable']:
                t0 = time.time()
                t0_sql = coffice.sql_db.sql_counter
                ...
                _logger.info("All post-tested in %.2fs, %s queries",
                             time.time() - t0, coffice.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the coffice http server and cron processor.
    """
    global server

    load_server_wide_modules()
    coffice.service.wsgi_server._patch_xmlrpc_marshaller()

    if coffice.evented:
        server = GeventServer(coffice.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(coffice.service.wsgi_server.application)
        ...
        server = ThreadedServer(coffice.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not coffice.evented:
        if inotify:
    ...
    if getattr(coffice, 'phoenix', False):
        _reexec()
