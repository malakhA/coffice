...
# User code must use the exceptions defined in ``coffice.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, coffice.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, coffice.tools.ustr(e.name))
    ...
    elif isinstance(e, coffice.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = coffice.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, coffice.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(coffice.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(coffice.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    # Historically, coffice is sending binary as base64 string.
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(coffice.tools.ustr(value), write)
...
def application_unproxied(environ, start_response):
    ...
    # coffice.service.web_services.objects_proxy.dispatch().
    ...
    with coffice.api.Environment.manage():
        result = coffice.http.root(environ, start_response)
