...
from coffice.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if coffice.tools.config['db_host']:
        env['PGHOST'] = coffice.tools.config['db_host']
    if coffice.tools.config['db_port']:
        env['PGPORT'] = str(coffice.tools.config['db_port'])
    if coffice.tools.config['db_user']:
        env['PGUSER'] = coffice.tools.config['db_user']
    if coffice.tools.config['db_password']:
        env['PGPASSWORD'] = coffice.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    ...
    """
    import coffice.modules as addons
    adps = coffice.addons.__path__
...
def _fileopen(path, mode, basedir, pathinfo, basename=None):
    ...
    import coffice.modules as addons
    paths = coffice.addons.__path__ + [config['root_path']]
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = coffice.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('coffice.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('coffice.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = coffice.fields.Datetime.from_string(value)
            value = coffice.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = coffice.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = coffice.fields.Datetime.from_string(value)
    else:
        timestamp = value
