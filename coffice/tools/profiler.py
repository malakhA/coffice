...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if not isinstance(in_self, coffice.models.BaseModel):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all coffice models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non coffice model from the log: [None])
        ...
        .. code-block:: python
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @profile()                  # log all methods for all coffice models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/openerp/coffice/coffice/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _cofficeProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_cofficeProfile, method)

    wrapper = decorator(_cofficeProfile, method)
    return wrapper
