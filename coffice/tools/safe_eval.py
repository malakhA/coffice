...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    except coffice.exceptions.except_orm:
        raise
    except coffice.exceptions.Warning:
        raise
    except coffice.exceptions.RedirectWarning:
        raise
    except coffice.exceptions.AccessDenied:
        raise
    except coffice.exceptions.AccessError:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except coffice.http.AuthenticationError:
        raise
