...
.. warning::
    ...
    This tutorial requires :ref:`having installed COffice <setup/install>`

Start/Stop the COffice server
==========================

COffice uses a client/server architecture in which clients are web browsers
accessing the COffice server via RPC.
...
In order to start the server, simply invoke the command :ref:`coffice-bin
...
Build an COffice module
====================

Both server and client extensions are packaged as *modules* which are
optionally loaded in a *database*.

COffice modules can either add brand new business logic to an COffice system, or
alter and extend existing business logic: a module can be created to add your
country's accounting rules to COffice's generic accounting support, while the
next module adds support for real-time visualisation of a bus fleet.

Everything in COffice thus starts and ends with modules.

Composition of a module
-----------------------

An COffice module can contain a number of elements:

Business objects
    Declared as Python classes, these resources are automatically persisted
    by COffice based on their configuration
...
are specified by using the :option:`--addons-path <coffice-bin --addons-path>`
option.
...
An COffice module is declared by its :ref:`manifest <reference/module/manifest>`.
...
COffice provides a mechanism to help set up a new module, :ref:`coffice-bin
<reference/cmdline/server>` has a subcommand :ref:`scaffold
<reference/cmdline/scaffold>` to create an empty module:

.. code-block:: console

    $ coffice-bin scaffold <module name> <where to put it>
...
.. exercise:: Module creation
    ...
    install it in COffice.

    .. only:: solutions

        #. Invoke the command ``coffice-bin scaffold openacademy addons``.
...
A key component of COffice is the :abbr:`ORM (Object-Relational Mapping)` layer.
...
:class:`~coffice.models.Model` which integrates them into the automated
persistence system.

Models can be configured by setting a number of attributes at their
definition. The most important attribute is
:attr:`~coffice.models.Model._name` which is required and defines the name for
the model in the COffice system. Here is a minimally complete definition of a
model::
...
:attr:`~coffice.fields.Field.string` (``unicode``, default: field's name)
    The label of the field in UI (visible by users).
:attr:`~coffice.fields.Field.required` (``bool``, default: ``False``)
    If ``True``, the field can not be empty, it must either have a default
    value or always be given a value when creating a record.
:attr:`~coffice.fields.Field.help` (``unicode``, default: ``''``)
    Long-form, provides a help tooltip to users in the UI.
:attr:`~coffice.fields.Field.index` (``bool``, default: ``False``)
    Requests that COffice create a `database index`_ on the column.
...
Example of simple fields are :class:`~coffice.fields.Boolean`,
:class:`~coffice.fields.Date`, :class:`~coffice.fields.Char`.

Reserved fields
###############

COffice creates a few fields in all models\ [#autofields]_. These fields are
managed by the system and shouldn't be written to. They can be read if
useful or necessary:

:attr:`~coffice.fields.Model.id` (:class:`~coffice.fields.Id`)
    The unique identifier for a record in its model.
:attr:`~coffice.fields.Model.create_date` (:class:`~coffice.fields.Datetime`)
    Creation date of the record.
:attr:`~coffice.fields.Model.create_uid` (:class:`~coffice.fields.Many2one`)
    User who created the record.
:attr:`~coffice.fields.Model.write_date` (:class:`~coffice.fields.Datetime`)
    Last modification date of the record.
:attr:`~coffice.fields.Model.write_uid` (:class:`~coffice.fields.Many2one`)
    user who last modified the record.

Special fields
##############

By default, COffice also requires a ``name`` field on all models for various
display and search behaviors. The field used for these purposes can be
overridden by setting :attr:`~coffice.models.Model._rec_name`.
...
COffice is a highly data driven system. Although behavior is customized using
Python_ code part of a module's value is in the data it sets up when loaded.

.. tip:: some modules exist solely to add data into COffice
    :class: aphorism
...
* ``model`` is the name of the COffice model for the record.
...
.. tip:: The content of the data files is only loaded when a module is
    ...
    :ref:`coffice-bin -u openacademy <reference/cmdline>` to save the changes
    to your database.
...
If no search view exists for the model, COffice generates one which only allows
...
:class:`Many2one(other_model, ondelete='set null') <coffice.fields.Many2one>`
...
:class:`One2many(other_model, related_field) <coffice.fields.One2many>`
    A virtual relationship, inverse of a :class:`~coffice.fields.Many2one`.
    A :class:`~coffice.fields.One2many` behaves as a container of records,
    ...
    .. danger::
        ...
        Because a :class:`~coffice.fields.One2many` is a virtual relationship,
        there *must* be a :class:`~coffice.fields.Many2one` field in the
        :samp:`{other_model}`, and its name *must* be :samp:`{related_field}`

:class:`Many2many(other_model) <coffice.fields.Many2many>`
...
COffice provides two *inheritance* mechanisms to extend an existing model in a
modular way.
...
.. seealso::
    ...
    * :attr:`~coffice.models.Model._inherit`
    * :attr:`~coffice.models.Model._inherits`

View inheritance
----------------

Instead of modifying existing views in place (by overwriting them), COffice
...
In COffice, :ref:`reference/orm/domains` are values that encode conditions on
...
:attr:`~coffice.fields.Field.compute` to the name of a method. The computation
...
.. code-block:: python
    ...
    import random
    from coffice import models, fields, api
...
on the compute method with the decorator :func:`~coffice.api.depends`.
    ...
    * Add a field ``active`` in the class Session, and set sessions as active by
    ...
    .. only:: solutions
        ...
        .. note::
            ...
            COffice has built-in rules making fields with an ``active`` field set
            to ``False`` invisible.
...
the record in the form view, and decorate it with :func:`~coffice.api.onchange`
...
COffice provides two ways to set up automatically verified invariants:
:func:`Python constraints <coffice.api.constrains>` and
:attr:`SQL constraints <coffice.models.Model._sql_constraints>`.

A Python constraint is defined as a method decorated with
:func:`~coffice.api.constrains`, and invoked on a recordset. The decorator
...
raise an exception if its invariant is not satisfied::
    ...
    from coffice.exceptions import ValidationError
...
:attr:`~coffice.models.Model._sql_constraints`. The latter is assigned to a list
...
.. exercise:: Add access control through the COffice interface
...
the same convention as the method :meth:`~coffice.models.Model.write` of the ORM.
...
:class:`~coffice.models.TransientModel` instead of
:class:`~coffice.models.Model`. The class
:class:`~coffice.models.TransientModel` extends :class:`~coffice.models.Model`
...
pt_BR.po). Translations will be loaded automatically by COffice for all
enabled languages. Developers always use English when creating a module, then
export the module terms using COffice's gettext POT export feature
...
.. tip:: The Portable Object files generated by COffice are published on
         `Transifex <https://www.transifex.com/coffice/public/>`__, making it
...
.. code-block:: text
   ...
   |- idea/ # The module directory
      |- i18n/ # Translation files
         | - idea.pot # Translation Template (exported from COffice)
...
.. tip:: 
   ...
   By default COffice's POT export only extracts labels inside XML files or
   inside field definitions in Python code, but any Python string can be
   translated this way by surrounding it with the function :func:`coffice._`
   (e.g. ``_("Label")``)

.. exercise:: Translate a module

   Choose a second language for your COffice installation. Translate your
   module using the facilities provided by COffice.
   ...
   .. only:: solutions
        ...
        #. In ``models.py``, add an import statement for the function
           ``coffice._`` and mark missing strings as translatable
...
COffice uses a report engine based on :ref:`reference/qweb`,
...
COffice is accessible through XML-RPC/JSON-RPC interfaces, for which libraries
exist in many languages.

XML-RPC Library
---------------

The following example is a Python 3 program that interacts with an COffice
...
.. exercise:: Add a new service to the client
   ...
   COffice (yours, or your instructor's). This program should display all
...
The following example is a Python 3 program that interacts with an COffice server
...
.. note::
    ...
    There are a number of high-level APIs in various languages to access COffice
    systems without *explicitly* going through XML-RPC or JSON-RPC, such as:

    * https://github.com/akretion/ooor
    * https://github.com/OCA/cofficerpc
    * https://github.com/nicolas-van/openerp-client-lib
    * http://pythonhosted.org/COfficeRPC
    * https://github.com/abhishek-jaiswal/php-openerp-lib

.. [#autofields] it is possible to :attr:`disable the automatic creation of some
                 fields <coffice.models.Model._log_access>`
.. [#rawsql] writing raw SQL queries is possible, but requires care as it
             bypasses all COffice authentication and security mechanisms.
