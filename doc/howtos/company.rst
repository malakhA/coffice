...
.. warning::
    ...
    This tutorial requires good knowledge of COffice.
...
To correctly manage multi-company behaviors, COffice's ORM provides multiple features:
...
.. note:: The `_compute_display_info` method is decorated with `depends_context('force_company')`
          (see :attr:`~coffice.api.depends_context`) to ensure that the computed field is recomputed
...
On each :meth:`~coffice.models.Model.create` and :meth:`~coffice.models.Model.write`, automatic checks
