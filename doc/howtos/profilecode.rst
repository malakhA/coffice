===================
Profiling COffice code
===================

.. warning::

    This tutorial requires :ref:`having installed COffice <setup/install>`
    and :doc:`writing COffice code <backend>`

Graph a method
==============

COffice embeds a profiler of code. This embeded profiler output can be used to
...
.. code:: python
    ...
    from coffice.tools.misc import profile
    [...]
...
.. code:: python
    ...
    from coffice.tools.profiler import profile
    [...]
    @profile
    @api.model
    def mymethod(...):
...
.. code:: bash
    ...
    2018-03-28 06:18:23,196 22878 INFO openerp coffice.tools.profiler:
    calls     queries   ms
    project.task ------------------------ /home/coffice/src/coffice/addons/project/models/project.py, 638
...
Sending the SIGQUIT signal to an COffice process (only available on POSIX) makes
this process output the current stack trace to log, with info level. When an
coffice process seems stucked, sending this signal to the process permit to know
what the process is doing, and letting the process continue his job.

Tracing code execution
======================

Instead of sending the SIGQUIT signal to an COffice process often enough, to check
...
where <PID> is the process ID of the coffice process you want to graph. This will
