...
This guide is about creating modules for COffice's web client.

To create websites with COffice, see :doc:`website`; to add business capabilities
or extend existing business systems of COffice, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed COffice <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple COffice module holding basic web component
...
.. code-block:: console
    ...
    $ git clone http://github.com/coffice/petstore

This will create a ``petstore`` folder wherever you executed the command.
You then need to add that folder to COffice's
:option:`addons path <coffice-bin --addons-path>`, create a new database and
...
Files used in the "web" side of an COffice module must be placed in a ``static``
...
``oepetstore/static/js/petstore.js``
    ...
    currently look like::
        ...
        coffice.oepetstore = function(instance, local) {
...
.. warning::
    ...
    "developer mode": log into your COffice instance (user *admin* password
    *admin* by default) open the user menu (in the top-right corner of the
    COffice screen) and select :guilabel:`About COffice` then :guilabel:`Activate
    the developer mode`:

    .. image:: web/about_coffice.png
        :align: center
...
The COffice framework uses one such pattern to define modules within web addons,
...
In COffice web, modules are declared as functions set on the global ``coffice``
...
* the first parameter is the current instance of the COffice web client, it gives
  access to various capabilities defined by the COffice (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the COffice web client needs to call them
...
For simplicity and developer-friendliness COffice web provides a class
system based on John Resig's `Simple JavaScript Inheritance`_.

New classes are defined by calling the :func:`~coffice.web.Class.extend`
method of :class:`coffice.web.Class`::
...
The :func:`~coffice.web.Class.extend` method takes a dictionary describing
...
by calling :func:`~coffice.web.Class.extend` on the parent class, as is done
to subclass :class:`~coffice.web.Class`::
...
The COffice web client bundles jQuery_ for easy DOM manipulation. It is useful
...
COffice Web makes specific components responsible for sections of a page. In
COffice web, the base for such components is the :class:`~coffice.Widget`
...
It extends :class:`~coffice.Widget` and overrides the standard method
:func:`~coffice.Widget.start`, which — much like the previous ``MyClass``
— does little for now.
...
The ``HomePage`` widget already has a :func:`~coffice.Widget.start`
...
All widgets have a :attr:`~coffice.Widget.$el` which represents the
section of page they're in charge of (as a jQuery_ object). Widget content
should be inserted there. By default, :attr:`~coffice.Widget.$el` is an
empty ``<div>`` element.
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear COffice user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in COffice Web, you will need to reload
    the page. There is no need to restart the COffice server.

The ``HomePage`` widget is used by COffice Web and managed automatically.
...
``GreetingsWidget``'s :func:`~coffice.Widget.appendTo` method::
...
* Finally it tells ``GreetingsWidget`` where to insert itself, delegating part
  of its :attr:`~coffice.Widget.$el` to the ``GreetingsWidget``.

When the :func:`~coffice.Widget.appendTo` method is called, it asks the
widget to insert itself at the specified position and to display its content.
The :func:`~coffice.Widget.start` method will be called during the call
to :func:`~coffice.Widget.appendTo`.
...
.. code-block:: html
    ...
    <div class="oe_petstore_homepage">
        <div>Hello dear COffice user!</div>
...
:class:`~coffice.Widget`, because we added some classes on them.
...
:func:`~coffice.Widget.getChildren`
...
When overriding the :func:`~coffice.Widget.init` method of a widget it is
...
it. This is done via the :func:`~coffice.Widget.destroy` method::

    greeting.destroy();

When a widget is destroyed it will first call
:func:`~coffice.Widget.destroy` on all its children. Then it erases itself
from the DOM. If you have set up permanent structures in
:func:`~coffice.Widget.init` or :func:`~coffice.Widget.start` which
must be explicitly cleaned up (because the garbage collector will not handle
them), you can override :func:`~coffice.Widget.destroy`.

.. danger::

    when overriding :func:`~coffice.Widget.destroy`, ``_super()``
...
As many other environments, COffice's solution is to use a `template engine`_.
COffice's template engine is called :ref:`reference/qweb`.
...
* It has special support in COffice Web's :class:`~coffice.Widget`, though it
  can be used outside of COffice's web client (and it's possible to use
  :class:`~coffice.Widget` without relying on QWeb)
...
.. note::
    ...
    like COffice :ref:`views <reference/views>`.
    ...
    characteristic of COffice, and losing it was considered unacceptable.
...
However, because :class:`~coffice.Widget` has special integration for QWeb
the template can be set directly on the widget via its
:attr:`~coffice.Widget.template` attribute::
...
* with the second version, the template is rendered right before
  :func:`~coffice.Widget.start` is called
...
.. warning::
    ...
    they're set as a widget's :attr:`~coffice.Widget.template`. If there are
...
When using :class:`~coffice.Widget`'s integration it is not possible to
...
before :func:`~coffice.Widget.start` is called (the widget's state will
essentially be that set up by :func:`~coffice.Widget.init`):
...
But because it's a common operation, :class:`~coffice.Widget` provides an
equivalent shortcut through the :func:`~coffice.Widget.$` method::
...
:attr:`~coffice.Widget.events` is an object (mapping) of an event to the
...
:func:`~coffice.Widget.trigger` takes the name of the event to trigger as
...
listening to the ``user_chose`` event using :func:`~coffice.Widget.on`::
...
:func:`~coffice.Widget.on` binds a function to be called when the
...
additional arguments of :func:`~coffice.Widget.trigger` if it has
any. Example::
...
.. note::
    ...
    exception to that rule is ``coffice.web.bus`` which exists specifically
    to broadcasts evens in which any widget could be interested throughout
    the COffice web application.
...
* :func:`~coffice.Widget.set` sets the value of a property and triggers
  :samp:`change:{propname}` (where *propname* is the property name passed as
  first parameter to :func:`~coffice.Widget.set`) and ``change``
* :func:`~coffice.Widget.get` retrieves the value of a property.
...
The class system of the COffice web framework allows direct modification of
existing classes using the :func:`~coffice.web.Class.include` method::
...
defined in the call to :func:`~coffice.web.Class.include`. This will also work
if some instances of the class (or of any of its sub-classes) were created
prior to the call to :func:`~coffice.Widget.include`.
...
In COffice, translations files are automatically generated by scanning the source
...
:func:`~coffice.web._t` (and also :func:`~coffice.web._lt`).
...
:func:`~coffice.web._lt` ("lazy translate") is similar but somewhat more
...
Communication with the COffice Server
==================================

Contacting Models
-----------------

Most operations with COffice involve communicating with *models* implementing
...
communicating with COffice requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, COffice web provides
...
The class used to call COffice models is :class:`coffice.Model`. It is
instantiated with the COffice model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~coffice.web.Model.call` can be used to call any (public) method of an
COffice model. It takes the following positional arguments:
...
:func:`~coffice.Widget.call` returns a deferred resolved with the value
...
the time zone of the user, used to compute correctly dates and times if COffice
...
solution to achieve that is to use :class:`coffice.web.CompoundContext`.

:class:`~coffice.web.CompoundContext` is a class used to pass the user's
...
dictionaries or other :class:`~coffice.web.CompoundContext` instances. It will
...
are related to the configuration of the current user in COffice plus the
``new_key`` key that was added when instantiating
:class:`~coffice.web.CompoundContext`.

Queries
-------

While :func:`~coffice.Model.call` is sufficient for any interaction with COffice
models, COffice Web provides a helper for simpler and clearer querying of models
(fetching of records based on various conditions):
:func:`~coffice.Model.query` which acts as a shortcut for the common
combination of :py:meth:`~coffice.models.Model.search` and
::py:meth:`~coffice.models.Model.read`. It provides a clearer syntax to search
and read models::
...
* :func:`~coffice.web.Model.query` takes an optional list of fields as
  parameter (if no field is provided, all fields of the model are fetched). It
  returns a :class:`coffice.web.Query` which can be further customized before
  being executed
* :class:`~coffice.web.Query` represents the query being built. It is
  ...
  :class:`~coffice.web.Query` for its customization options.

When the query is set up as desired, simply call
:func:`~coffice.web.Query.all` to execute it and return a
deferred to its result. The result is the same as
:py:meth:`~coffice.models.Model.read`'s, an array of dictionaries where each
...
.. exercise:: Pet Toys List
    ...
    In COffice, images are generally stored in regular fields encoded as
...
In COffice, many operations start from an :ref:`action <reference/actions>`:
...
In COffice Web, the component responsible for handling and reacting to these
...
:func:`~coffice.Widget.do_action` is a shortcut of :class:`~coffice.Widget`
...
the COffice web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for COffice web. The server simply sends
...
``instance.web.client_actions`` is a :class:`~coffice.web.Registry` in which
the action manager looks up client action handlers when it needs to execute
one. The first parameter of :class:`~coffice.web.Registry.add` is the name
(tag) of the client action, and the second parameter is the path to the widget
from the COffice web client root.
...
Much of COffice web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`COffice views <reference/views>` are implemented through a subclass
of :class:`coffice.web.View` which provides a bit of generic basic structure
for handling events and displaying model information.

The *search view* is considered a view type by the main COffice framework, but
...
:py:class:`~coffice.models.Model.fields_view_get`) and any other data source
it needs. To that purpose, views are provided with an optional view
identifier set as the :attr:`~coffice.web.View.view_id` attribute.

Views are also provided with a :class:`~coffice.web.DataSet` instance which
...
:func:`~coffice.web.View.do_search`, and updating their
:class:`~coffice.web.DataSet` as necessary.
...
:py:class:`~coffice.fields.Char` fields which should contain email addresses
...
* It must correctly implement the 3 field attributes available in all fields
  of COffice. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in COffice have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the COffice server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in COffice. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the COffice web
client.
...
user to modify it can be useful, but most fields in COffice also allow editing.
...
their :attr:`~coffice.web.form.FormWidget.field_manager` attribute::
...
:attr:`~coffice.web.form.FormWidget` is generally the
:class:`~coffice.web.form.FormView` itself, but features used from it should
be limited to those defined by :class:`~coffice.web.form.FieldManagerMixin`,
the most useful being:

* :func:`~coffice.web.form.FieldManagerMixin.get_field_value(field_name)`
  which returns the value of a field.
* :func:`~coffice.web.form.FieldManagerMixin.set_values(values)` sets multiple
