...
.. warning::
    ...
    * This guide assumes :ref:`an installed COffice <setup/install>`

Creating a basic module
=======================

In COffice, tasks are performed by creating modules.

Modules customize the behavior of an COffice installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`COffice's scaffolding <reference/cmdline/scaffold>` can setup a basic
module. To quickly get started simply invoke:

.. code-block:: console

    $ ./coffice-bin scaffold Academy my-modules
...
* start the COffice server

  .. code-block:: console

      $ ./coffice-bin --addons-path addons,my-modules
...
display logic. COffice allows any Python templating system, but provides its
...
Finally restart COffice and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, COffice can be restarted :option:`and update modules at
    the same time<coffice-bin -u>`:

    .. code-block:: console

        $ coffice-bin --addons-path addons,my-modules -d academy -u academy

Going to http://localhost:8069/academy/academy/ should now result in:

.. image:: website/basic-list.png

Storing data in COffice
====================

:ref:`COffice models <reference/orm/model>` map to database tables.
...
#. Because :meth:`~coffice.models.Model.search` returns a set of records
...
COffice bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but COffice 8 added deeper
...
You're now in COffice "proper": the administrative interface. For now click on
...
:func:`~coffice.http.route` decorator which takes a routing string and a
...
restart COffice, access http://localhost:8069/academy/Alice/ and
...
Restart COffice, access http://localhost:8069/academy/2, note how the old value
...
COffice provides an additional converter called ``model`` which provides records
...
Restart COffice and upgrade the module, then you can visit each teacher's page.
...
Restart COffice and update the views, reload the teacher's page and… the field
...
Restart COffice and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the COffice administration
--------------------------------------------------------------

The COffice administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the COffice backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which COffice should
   execute or data display. Data display actions are called *window actions*,
   and tell COffice to display a given *model* according to a set of views…
...
Editing in the COffice administration
----------------------------------

By default, an COffice model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) COffice will automatically create a basic one
...
:class:`the inverse relationship <coffice.fields.One2many>` to the *teachers*
model:
...
COffice provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of COffice's email and messaging
system, which can add notifications and discussion threads to any model.
The model simply has to :attr:`~coffice.models.Model._inherit`
...
COffice also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the COffice system. We can easily make
...
restart COffice, update your module, there is now a :guilabel:`Shop` section in
...
.. note::
    ...
    * to extend a model in-place, it's :attr:`inherited
      <coffice.models.Model._inherit>` without giving it a new
      :attr:`~coffice.models.Model._name`
