...
Official compatibility: COffice 11 will be the first LTS release to introduce
...
used by the official COffice distribution have been ported and are considered
...
.. important::
    ...
    a guide to write 2/3-compatible COffice code. It does not go through all the
    changes in Python but rather through issues which have been found in the
    standard COffice distribution in order to show how to evolve such code such
...
A cross compatible COffice would only support Python 2.7 and Python 3.5 and
...
* fixing dependencies
* using a ``collections.OrderedDict`` or ``coffice.tools.misc.OrderedSet`` instead
...
* ``urllib``, ``urllib2`` and ``urlparse`` were redistributed across
  ...
  Since `requests`_ and `werkzeug`_ are already hard dependencies of COffice,
...
* ``cgi.escape`` (HTML escaping) is deprecated in Python 3, prefer COffice's own
  :func:`coffice.tools.misc.html_encode`.
...
actually happened in COffice with :mod:`coffice.tools.mimetypes`.
...
However with respect to COffice it turns out most or all uses of ``csv`` fit
inside a model of *byte stream to and from text values*.

The latter is thus a model implemented by cross-version wrappers
:func:`coffice.tools.pycompat.csv_reader` and
:func:`coffice.tools.pycompat.csv_writer`: they take a *UTF-8 byte stream* and
