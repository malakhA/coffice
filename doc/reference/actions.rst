...
``views``
    ...
    :meth:`~coffice.models.Model.fields_view_get`). The first type of the
...
Allow opening a URL (website/web page) via an COffice action. Can be customized
via two fields:
...
::

    {
        ...
        "url": "https://coffice.com",
        "target": "self",
    }

will replace the current content section by the COffice home page.
...
.. autoclass:: coffice.addons.base.models.ir_actions.IrActionsServer
...
``model_id``
    COffice model linked to the action.
...
``fields_lines`` (create/write)
    ...
    :class:`~coffice.fields.One2many` with the fields:
...
``paperformat_id``
    :class:`~coffice.fields.Many2one` field to the paper format you wish to
