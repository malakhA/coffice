...
================================
Command-line interface: coffice-bin
================================
...
.. option:: -s, --save
    ...
    (:file:`{$HOME}/.cofficerc` by default, and can be overridden using
    :option:`-c`).
...
.. option:: --screenshots
    ...
    fails. It defaults to :file:`/tmp/coffice_tests/{db_name}/screenshots`
...
.. option:: --db-filter <filter>
    ...
    - ``%d`` is replaced by the subdomain the request is made on, with the
      exception of ``www`` (so domain ``coffice.com`` and ``www.coffice.com`` both
      match the database ``coffice``).

      These operations are case sensitive. Add option ``(?i)`` to match all
      databases (so domain ``coffice.com`` using ``(?i)%d`` matches the database
      ``COffice``).
    ...
    .. code-block:: bash
        ...
        $ coffice-bin --db-filter ^11.*$

    Restrict access to databases whose name starts with 11

    .. code-block:: bash

        $ coffice-bin --database 11firstdatabase,11seconddatabase

    Restrict access to only two databases, 11firstdatabase and 11seconddatabase

    .. code-block:: bash

        $ coffice-bin --database 11firstdatabase,11seconddatabase -u base
    ...
    .. code-block:: bash
        ...
        $ coffice-bin --db-filter ^11.*$ --database 11firstdatabase,11seconddatabase -u base
...
.. option:: --db_sslmode
    ...
    Control the SSL security of the connection between COffice and PostgreSQL.
...
.. option:: --email-from <address>
    ...
    Email address used as <FROM> when COffice needs to send mails
...
.. option:: --smtp-ssl
    ...
    If set, coffice should use SSL/STARTSSL SMTP connections
...
Use these options to translate COffice to another language. See i18n section of
...
By default, COffice displays all logging of level_ ``info`` except for workflow
...
.. option:: --log-handler <handler-spec>
    ...
    e.g. ``coffice.models:DEBUG`` will enable all logging messages at or above
    ...
    * If the level is omitted, the logger is set to ``INFO``
    ...
    .. code-block:: console
        ...
        $ coffice-bin --log-handler :DEBUG --log-handler werkzeug:CRITICAL --log-handler coffice.fields:WARNING

.. option:: --log-request

    enable DEBUG logging for RPC requests, equivalent to
    ``--log-handler=coffice.http.rpc.request:DEBUG``

.. option:: --log-response

    enable DEBUG logging for RPC responses, equivalent to
    ``--log-handler=coffice.http.rpc.response:DEBUG``

.. option:: --log-web

    enables DEBUG logging of HTTP requests and responses, equivalent to
    ``--log-handler=coffice.http:DEBUG``

.. option:: --log-sql

    enables DEBUG logging of SQL querying, equivalent to
    ``--log-handler=coffice.sql_db:DEBUG``
...
.. option:: --log-level <level>
    ...
    ``coffice`` and ``werkzeug`` loggers (except for ``debug`` which is only
    set on ``coffice``).

    COffice also provides debugging pseudo-levels which apply to different sets
    of loggers:
    ...
    ``debug_rpc``
        sets the ``coffice`` and HTTP request loggers to ``debug``

        equivalent to ``--log-level debug --log-request``
    ``debug_rpc_answer``
        sets the ``coffice`` and HTTP request and response loggers to
        ``debug``
...
The default configuration file is :file:`{$HOME}/.cofficerc` which
can be overridden using :option:`--config <coffice-bin -c>`. Specifying
:option:`--save <coffice-bin -s>` will save the current configuration state back
to that file.
...
COffice command-line also allows to launch coffice as a python console environment.
...
.. program:: coffice-bin scaffold

Scaffolding is the automated creation of a skeleton structure to simplify
bootstrapping (of new modules, in the case of COffice). While not necessary it
avoids the tedium of setting up basic structures and looking up what all
starting requirements are.

Scaffolding is available via the :command:`coffice-bin scaffold` subcommand.
...
.. code-block:: console
    ...
    $ coffice_bin scaffold my_module /addons/
