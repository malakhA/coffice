...
COffice is greatly data-driven, and a big part of modules definition is thus
...
The main way to define data in COffice is via XML data files: The broad structure
of an XML data file is the following:

* Any number of operation elements within the root element ``coffice``
...
.. note::
    ...
    can specify the coffice flag ``noupdate`` set to 1.  If part of
    ...
    .. code-block:: xml
      ...
      <coffice>
          <data noupdate="1">
              <!-- Only loaded when installing the module (coffice-bin -i module) -->
              <operation/>
          </data>

          <!-- (Re)Loaded at install and update (coffice-bin -i/-u) -->
          <operation/>
      </coffice>
...
``search``
    ...
    the field is a :class:`~coffice.fields.Many2one`
``ref``
    ...
    Mostly for :class:`~coffice.fields.Many2one` and
    :class:`~coffice.fields.Reference` fields
``type``
...
Because some important structural models of COffice are complex and involved,
...
.. literalinclude:: ../../coffice/addons/base/data/res.country.state.csv
    :language: text
    :lines: 1-15
...
.. csv-table::
    :file: ../../coffice/addons/base/data/res.country.state.csv
    :header-rows: 1
