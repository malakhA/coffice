:banner: banners/coffice_guideline.jpg
...
This page introduces the COffice Coding Guidelines. Those aim to improve the
quality of COffice Apps code. Indeed proper code improves readability, eases
...
.. warning::
    ...
    details, see our `pull request guide <https://coffice.com/submit-pr>`_.
...
File naming is important to quickly find information through all coffice addons.
This section explains how to name files in a standard coffice module. As an
example we use a `plant nursery <https://github.com/tivisse/cofficedays-2018/tree/master/plant_nursery>`_ application.
...
.. code-block:: text
    ...
    |   |-- res_partner.py (inherited COffice model)
...
contained in a file named ``<module_name>.py``. An old convention in COffice is to
...
data (image, libraries) outside COffice: do not use an URL to an image but copy
...
directory. This naming comes from old coffice applications using the wizard
...
The complete tree of our COffice module therefore looks like
...
- The tag *<data>* is only used to set not-updatable data with ``noupdate=1``.
  ...
  set on the ``<coffice>`` tag and do not set a ``<data>`` tag.
...
COffice supports custom tags acting as syntactic sugar:
...
Using a linter can help show syntax and semantic warnings or errors. COffice
...
#. Imports of ``coffice``
#. Imports from COffice modules (rarely, and only if necessary)
...
.. code-block:: python
    ...
    # 2 : imports of coffice
    import coffice
    from coffice import api, fields, models, _ # alphabetically ordered
    from coffice.tools.safe_eval import safe_eval as eval
    # 3 : imports from coffice addons
    from coffice.addons.website.models.website import slug
    from coffice.addons.web.controllers.main import login_redirect
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The COffice framework is in charge of providing the transactional context for
...
COffice uses a GetText-like method named "underscore" ``_( )`` to indicate that
...
In general in COffice, when manipulating strings, prefer ``%`` over ``.format()``
...
- Model name (using the dot notation, prefix by the module name) :
    - When defining an COffice Model : use singular form of the name (*res.partner*
      and *sale.order* instead of *res.partnerS* and *saleS.orderS*)
    - When defining an COffice Transient (wizard) : use ``<related_base_model>.<action>``
...
- COffice Python Class : use camelcase (Object-oriented style).
...
COffice addons have some conventions on how to structure various files. We explain
here in more details how web assets are supposed to be organized.

The first thing to know is that the COffice server will serve (statically) all files
...
statically available at the url *your-coffice-server.com/web/static/src/js/some_file.js*
...
More precise JS guidelines are detailed in the `github wiki  <https://github.com/coffice/coffice/wiki/Javascript-coding-guidelines>`_.
...
- **[CLA]** for signing the COffice Individual Contributor License;
...
For COffice R&D developers "PO team asked me to do it" is not a valid why, by the way.
...
If you are an COffice R&D developer the WHY should be the purpose of the task you
