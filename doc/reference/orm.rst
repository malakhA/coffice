...
.. autoclass:: coffice.models.BaseModel()
    ...
    .. seealso:: :class:`coffice.models.AbstractModel`

    .. autoattribute:: _transient

    .. seealso:: :class:`coffice.models.TransientModel`
...
.. autoclass:: coffice.models.AbstractModel()

Model
-----

.. autoclass:: coffice.models.Model()

TransientModel
--------------

.. autoclass:: coffice.models.TransientModel()
...
application, they are heavily used in many popular COffice applications such as
logistics or accounting and their misuse can create invisible yet painful
bugs, this excerpt aims to provide COffice developers with the knowledge required
...
into a compatible type: :func:`~coffice.fields.Date.to_date` will convert to a `datetime.date`
object while :func:`~coffice.fields.Datetime.to_datetime` will convert to a `datetime.datetime`.
...
:class:`~coffice.fields.Date` and :class:`~coffice.fields.Datetime`.
These helpers are also available by importing `coffice.tools.date_utils`.

.. note:: Timezones

    Datetime fields are stored as `timestamp without timezone` columns in the database and are stored
    in the UTC timezone. This is by design, as it makes the COffice database independent from the timezone
...
:func:`~coffice.api.depends`. ::

    from coffice import api
...
.. attribute:: id
    ...
    Identifier :class:`field <coffice.fields.Field>`
...
.. attribute:: create_date
    ...
    :class:`~coffice.fields.Datetime`

.. attribute:: create_uid

    :class:`~coffice.fields.Many2one`
...
.. attribute:: name
  ...
  default value for :attr:`~coffice.models.BaseModel._rec_name`, used to
  display records in context where a representative "naming" is
  necessary.

  :class:`~coffice.fields.Char`
...
..   in list views.
..
..   :class:`~coffice.fields.Integer`

.. attribute:: state

  lifecycle stages of the object, used by the ``states`` attribute on
  :class:`fields <coffice.fields.Field>`.

  :class:`~coffice.fields.Selection`
...
.. attribute:: company_id
  ...
  Main field name used for COffice multi-company behavior.

  Used by `:meth:~coffice.models._check_company` to check multi company consistency.
  Defines whether a record is shared between companies (no value) or only
  accessible by the users of a given company.

  :class:`~coffice.fields.Many2one`
  :type: :class:`~coffice.addons.base.models.res_company`
...
.. note::
    ...
    records, use :meth:`~coffice.models.BaseModel.mapped`::
...
Accessing a relational field (:class:`~coffice.fields.Many2one`,
:class:`~coffice.fields.One2many`, :class:`~coffice.fields.Many2many`)
*always* returns a recordset, empty if the field is not set.

Record cache and prefetching
----------------------------

COffice maintains a cache for the fields of the records, so that not every field
...
To avoid reading one field on one record at a time, COffice *prefetches* records
...
The :class:`~coffice.api.Environment` stores various contextual data used by
...
using :attr:`~coffice.models.Model.env` and gives access to:

* the current user (:attr:`~coffice.api.Environment.user`)
* the cursor (:attr:`~coffice.api.Environment.cr`)
* the superuser flag (:attr:`~coffice.api.Environment.su`)
* or the context (:attr:`~coffice.api.Environment.context`)
...
The :attr:`~coffice.api.Environment.cr` attribute on environments is the
...
Because models use the same cursor and the :class:`~coffice.api.Environment`
...
.. note::
    ...
    :meth:`~coffice.models.Model.invalidate_cache` method.

.. automethod:: Model.invalidate_cache

.. warning::
    Executing raw SQL bypasses the ORM, and by consequent, COffice security rules.
...
* ``field_name`` (``str``)
    ...
    a :class:`~coffice.fields.Many2one` using dot-notation e.g. ``'street'``
...
* ``operator`` (``str``)
    ...
    ``child_of``
        ...
        :attr:`~coffice.models.Model._parent_name`).
    ``parent_of``
...
.. attribute:: env
    ...
    :type: :class:`~coffice.api.Environment`
...
COffice provides three different mechanisms to extend models in a modular way:
...
When using the :attr:`~coffice.models.Model._inherit` and
:attr:`~coffice.models.Model._name` attributes together, COffice creates a new
model using the existing one (provided via
:attr:`~coffice.models.Model._inherit`) as a base. The new model gets all the
fields, methods and meta-information (defaults & al) from its base.

.. literalinclude:: ../../coffice/addons/test_documentation_examples/inheritance.py
    :language: python
    :lines: 6-

and using them:

.. literalinclude:: ../../coffice/addons/test_documentation_examples/tests/test_inheritance.py
    :language: python
    :lines: 10,11,14,19
...
When using :attr:`~coffice.models.Model._inherit` but leaving out
:attr:`~coffice.models.Model._name`, the new model replaces the existing one,
...
.. literalinclude:: ../../coffice/addons/test_documentation_examples/extension.py
    :language: python
    :lines: 6-

.. literalinclude:: ../../coffice/addons/test_documentation_examples/tests/test_extension.py
    :language: python
    :lines: 10,15
...
at runtime) but less power: using the :attr:`~coffice.models.Model._inherits`
...
:class:`~coffice.fields.Reference` fields automatically set up on the parent
model.
...
.. literalinclude:: ../../coffice/addons/test_documentation_examples/delegation.py
    :language: python
    :lines: 5-

.. literalinclude:: ../../coffice/addons/test_documentation_examples/tests/test_delegation.py
    :language: python
    :lines: 11-14,23,28
