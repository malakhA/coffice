...
QWeb is the primary templating_ engine used by COffice\ [#othertemplates]_. It
...
:meth:`coffice.http.HttpRequest.render`:
...
This automatically creates a :class:`~coffice.http.Response` object which can
...
.. py:method:: render(cr, uid, id[, values][, engine='ir.qweb][, context])
    ...
    ``request``
        the current :class:`~coffice.http.WebRequest` object, if any
    ``debug``
    ...
    :param str engine: name of the COffice model to use for rendering, can be
...
.. .. automodule:: coffice.addons.base.ir.ir_qweb
...
.. js:class:: QWeb2.Engine
    ...
    COffice Web instantiates one for the user in the core module, and
    ...
    COffice Web, Kanban views get their own :js:class:`QWeb2.Engine`
    ...
    .. js:attribute:: QWeb2.Engine.preprocess_node
        ...
        node to template code. In COffice Web, this is used to
...
.. [#othertemplates] although it uses a few others, either for historical
                     ...
                     use case. COffice 9.0 still depends on Jinja_ and Mako_.
