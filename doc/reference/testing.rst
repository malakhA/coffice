...
There are many ways to test an application.  In COffice, we have three kinds of
tests
...
COffice provides support for testing modules using unittest.
...
`unittest documentation`_, but COffice provides a number of utilities and helpers
related to testing COffice content (modules, mainly):

.. autoclass:: coffice.tests.common.TransactionCase
    :members: browse_ref, ref

.. autoclass:: coffice.tests.common.SingleTransactionCase
    :members: browse_ref, ref

.. autoclass:: coffice.tests.common.SavepointCase

.. autoclass:: coffice.tests.common.HttpCase
    :members: browse_ref, ref, url_open, phantom_js

.. autofunction:: coffice.tests.common.tagged
...
.. autofunction:: coffice.tests.common.at_install

.. autofunction:: coffice.tests.common.post_install

The most common situation is to use
:class:`~coffice.tests.common.TransactionCase` and test a property of a model
in each method::
...
.. autoclass:: coffice.tests.common.Form
    :members:

.. autoclass:: coffice.tests.common.M2MProxy
    :members: add, remove, clear

.. autoclass:: coffice.tests.common.O2MProxy
...
:option:`--test-enable <coffice-bin --test-enable>` was enabled when starting the
COffice server.
...
In COffice, Python tests can be tagged to facilitate the test selection when
running tests.

Subclasses of :class:`coffice.tests.common.BaseCase` (usually through
:class:`~coffice.tests.common.TransactionCase`,
:class:`~coffice.tests.common.SavepointCase` or
:class:`~coffice.tests.common.HttpCase`) are automatically tagged with
``standard``, ``at_install`` and their source module's name by default.

Invocation
^^^^^^^^^^

:option:`--test-tags <coffice-bin --test-tags>` can be used to select/filter tests
to run on the command-line.

This option defaults to ``+standard`` meaning tests tagged ``standard``
(explicitly or implicitly) will be run by default when starting COffice
with :option:`--test-enable <coffice-bin --test-enable>`.

When writing tests, the :func:`~coffice.tests.common.tagged` decorator can be
used on **test classes** to add or remove tags.

The decorator's arguments are tag names, as strings.

.. danger:: :func:`~coffice.tests.common.tagged` is a class decorator, it has no
...
.. code-block:: python
    ...
    from coffice.tests import TransactionCase, tagged
...
.. code-block:: console
    ...
    $ coffice-bin --test-enable --test-tags nice
...
:option:`--test-tags <coffice-bin --test-tags>`: on the command-line, values
are *additive* (you're selecting all tests with *any* of the specified tags)

.. code-block:: console

    $ coffice-bin --test-enable --test-tags nice,standard
...
.. code-block:: console
    ...
    $ coffice-bin --test-enable --test-tags 'standard,-slow'

When you write a test that does not inherit from the
:class:`~coffice.tests.common.BaseCase`, this test will not have the default tags,
...
- ``standard``: All COffice tests that inherit from
  :class:`~coffice.tests.common.BaseCase` are implicitely tagged standard.
  :option:`--test-tags <coffice-bin --test-tags>` also defaults to ``standard``.
...
- *module_name*: COffice tests classes extending
  :class:`~coffice.tests.common.BaseCase` are implicitely tagged with the
  ...
  .. code-block:: console
      ...
      $ coffice-bin --test-enable --test-tags stock_account
...
.. important::
    ...
    modules have to be selected with the :option:`-u <coffice-bin -u>` or
    :option:`-i <coffice-bin -i>` switches.  For simplicity, those switches are
...
.. code-block:: console
    ...
    $ coffice-bin --test-enable --test-tags sale

Run the tests from the sale module but not the ones tagged as slow:

.. code-block:: console

    $ coffice-bin --test-enable --test-tags 'sale,-slow'

Run only the tests from stock or tagged as slow:

.. code-block:: console

    $ coffice-bin --test-enable --test-tags '-standard, slow, stock'
...
guarantee that some basic functionality still works. Since COffice has a non trivial
...
The COffice framework uses the QUnit_ library testing framework as a test runner.
...
The main way to run the test suite is to have a running COffice server, then
...
With the way COffice is designed, any addon can modify the behaviour of other parts
...
1. create a new file *my_addon/static/tests/utils_tests.js*. This file contains the basic code to
   add a QUnit module *my_addon > utils*.
    ...
    .. code-block:: javascript
        ...
        coffice.define('my_addon.utils_tests', function (require) {
        "use strict";
...
Without help, it is quite difficult to test some parts of COffice. In particular,
...
- Mock test functions: these functions help setting up a test environment. The
  most important use case is mocking the answers given by the COffice server. These
...
- `qunit assertions`_: QUnit can be extended with specialized assertions. For
  COffice, we frequently test some DOM properties. This is why we made some
...
.. code-block:: console
  ...
  '/tmp/coffice_tests/{db_name}/screenshots/'

Two new command line arguments were added since COffice 13.0 to control this behavior:
:option:`--screenshots <coffice-bin --screenshots>` and :option:`--screencasts <coffice-bin --screencasts>`


.. _qunit: https://qunitjs.com/
.. _qunit_config.js: https://github.com/coffice/coffice/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/qunit_config.js#L49
.. _web.js_tests_assets: https://github.com/coffice/coffice/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/views/webclient_templates.xml#L427
.. _web.qunit_suite: https://github.com/coffice/coffice/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/views/webclient_templates.xml#L509
.. _controller: https://github.com/coffice/coffice/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/controllers/main.py#L637
.. _test_js.py: https://github.com/coffice/coffice/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/tests/test_js.py#L13
.. _test_utils.js: https://github.com/coffice/coffice/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/test_utils.js
.. _mock server: https://github.com/coffice/coffice/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/mock_server.js
.. _qunit assertions: https://github.com/coffice/coffice/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/qunit_asserts.js
.. _createView: https://github.com/coffice/coffice/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/test_utils_create.js#L267
