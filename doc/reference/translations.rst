...
.. note:: If you want to contribute to the translation of COffice itself, please refer to the
  `COffice Wiki page <https://github.com/coffice/coffice/wiki/Translations>`_.
...
automatically loaded by COffice when the corresponding language is installed (via
...
COffice automatically exports translatable strings from "data"-type content:
...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~coffice.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~coffice.models.Model._constraints` and
  :attr:`~coffice.models.Model._sql_constraints` are exported

Explicit exports
================

When it comes to more "imperative" situations in Python code or Javascript
code, COffice cannot automatically export translatable terms so they
must be marked explicitly for export. This is done by wrapping a literal
string in a function call.

In Python, the wrapping function is :func:`coffice._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`coffice.web._t`:
...
The lazy version of `_` and `_t` is :func:`coffice._lt` in python and
:js:func:`coffice.web._lt` in javascript. The translation lookup is executed only
