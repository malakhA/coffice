...
``groups_id``
    :class:`~coffice.fields.Many2many` field to the groups allowed to view/use
    the current view
``inherit_id``
...
``banner_route``
    ...
    .. code-block:: python
        ...
        class MyController(coffice.http.Controller):
...
``button``
    ...
    ``type``
        type of button, indicates how it clicking it affects COffice:
    ...
    ``context``
        merged into the view's context when performing the button's COffice call
    ``confirm``
        confirmation message to display (and for the user to accept) before
        performing the button's COffice call
...
Semantic components tie into and allow interaction with the COffice
system. Available semantic components are:

``button``
  call into the COffice system, similar to :ref:`list view buttons
...
``field``
  ...
  ``widget``
    ...
    (e.g. :class:`~coffice.fields.Char`,
    :class:`~coffice.fields.Many2one`). The ``widget`` attributes allows using
  ...
  ``on_change``
    ...
    .. deprecated:: 8.0
       ...
       Use :func:`coffice.api.onchange` on the model
  ...
  ``mode``
    for :class:`~coffice.fields.One2many`, display mode (view type) to use for
  ...
  ``filename``
    ...
    file
  ``password``
    indicates that a :class:`~coffice.fields.Char` field stores a password and
...
``examples``
  if set to a key in the `KanbanExamplesRegistry`_, examples on column setups will be available in the grouped kanban view. `Here <https://github.com/coffice/coffice/blob/99821fdcf89aa66ac9561a972c6823135ebf65c0/addons/project/static/src/js/project_task_kanban_examples.js#L27>`_ is an example of how to define those setups.
``group_create``
...
``templates``
  ...
  ``record``
    ...
    a :meth:`~coffice.models.Model.read` (except for date and datetime fields
    that are `formatted according to user's locale
    <https://github.com/coffice/coffice/blob/a678bd4e/addons/web_kanban/static/src/js/kanban_record.js#L102>`_)
  ``context``
  ...
  ``selection_mode``
    ...
    * buttons and links with a ``type`` attribute become perform COffice-related
      operations rather than their standard HTML function. Possible types are:

      ``action``, ``object``
        standard behavior for :ref:`COffice buttons
        <reference/views/list/button>`, most attributes relevant to standard
        COffice buttons can be used.
      ``open``
...
``arrow`` (required, 1)
    ...
    ``object`` (required)
      the edge's COffice model
    ``source`` (required)
      :class:`~coffice.fields.Many2one` field of the edge's model pointing to
...
.. warning::
   ...
   The Dashboard view is only available in COffice Enterprise.
...
``aggregate``
    ...
    - ``group_operator`` (optional)
        ...
        .. note:: The special aggregate function ``count_distinct`` (defined in coffice) can also be used here
...
.. warning::
   ...
   The Cohort view is only available in COffice Enterprise.
...
``field``
    ...
    ``context``
        ...
        values e.g. ``[id_1, id_2]`` for a :class:`~coffice.fields.Many2one` field).
    ...
    ``widget``
        ...
        standard COffice 8.0 is a ``selection`` widget for
        :class:`~coffice.fields.Many2one` fields)
    ``domain``
        if the field can provide an auto-completion
        (e.g. :class:`~coffice.fields.Many2one`), filters the possible
        completion results.
...
``filter``
    ...
    ``domain`` (optional)
        an COffice :ref:`domain <reference/orm/domains>`, will be appended to the
...
.. warning::
   ...
   The Map view is only available in COffice Enterprise
...
.. _KanbanExamplesRegistry: https://github.com/coffice/coffice/blob/99821fdcf89aa66ac9561a972c6823135ebf65c0/addons/web/static/src/js/views/kanban/kanban_examples_registry.js
