:banner: banners/deploying_coffice.jpg

==============
Deploying COffice
==============

This document describes basic steps to set up COffice in production or on an
...
COffice is a multi-tenant system: a single COffice system may run and serve a number
...
bound to a database: COffice needs to know which database should be used to load
...
accessible COffice needs a rule to know which one it should use.

That is one of the purposes of :option:`--db-filter <coffice-bin --db-filter>`:
...
.. note::
  Setting a proper :option:`--db-filter <coffice-bin --db-filter>` is an important part
...
UNIX socket is fine if you want COffice and PostgreSQL to execute on the same
machine, and is the default when no host is provided, but if you want COffice and
...
* Only accept loopback connections and `use an SSH tunnel`_ between the
  machine on which COffice runs and the one on which PostgreSQL runs, then
  configure COffice to connect to its end of the tunnel
* Accept connections to the machine on which COffice is installed, possibly
  over ssl (see `PostgreSQL connection settings`_ for details), then configure
  COffice to connect over the network
...
.. _setup/deploy/coffice:

Configuring COffice
----------------

Out of the box, COffice connects to a local postgres over UNIX socket via port
...
create a new user (``coffice``) and set it as the database user.
...
SSL Between COffice and PostgreSQL
-------------------------------

Since COffice 11.0, you can enforce ssl connection between COffice and PostgreSQL.
in COffice the db_sslmode control the ssl security of the connection
...
COffice includes built-in HTTP servers, using either multithreading or
multiprocessing.
...
* Multiprocessing is enabled by configuring :option:`a non-zero number of
  worker processes <coffice-bin --workers>`, the number of workers should be based
...
listening on :option:`the longpolling port <coffice-bin --longpolling-port>` but
the client will not connect to it.

Instead you must have a proxy redirecting requests whose URL starts with
``/longpolling/`` to the longpolling port. Other request should be proxied to
the :option:`normal HTTP port <coffice-bin --http-port>`

To achieve such a thing, you'll need to deploy a reverse proxy in front of COffice,
like nginx or apache. When doing so, you'll need to forward some more http Headers
to COffice, and activate the proxy_mode in COffice configuration to have COffice read those
headers.
...
* RAM = 9 * ((0.8*150) + (0.2*1024)) ~= 3Go RAM for COffice
...
Whether it's accessed via website/web client or web service, COffice transmits
authentication information in cleartext. This means a secure deployment of
COffice must use HTTPS\ [#switching]_. SSL termination can be implemented via
just about any SSL termination proxy, but requires the following setup:

* Enable COffice's :option:`proxy mode <coffice-bin --proxy-mode>`. This should only be enabled when COffice is behind a reverse proxy
...
in ``/etc/nginx/sites-enabled/coffice.conf`` set:
...
.. code-block:: nginx
  ...
  server {
     listen 80;
     server_name coffice.mycompany.com;
  ...
  server {
   ...
   # Add Headers for coffice proxy mode
   ...
   # log
   access_log /var/log/nginx/coffice.access.log;
   error_log /var/log/nginx/coffice.error.log;

   # Redirect longpoll requests to coffice longpolling port
   location /longpolling {
   proxy_pass http://cofficechat;
   }

   # Redirect requests to coffice backend server
   location / {
     proxy_redirect off;
     proxy_pass http://coffice;
   }
...
COffice as a WSGI Application
==========================

It is also possible to mount COffice as a standard WSGI_ application. COffice
provides the base for a WSGI launcher script as ``coffice-wsgi.example.py``. That
script should be customized (possibly after copying it from the setup directory) to correctly set the
configuration directly in :mod:`coffice.tools.config` rather than through the
command-line or a configuration file.

However the WSGI server will only expose the main HTTP endpoint for the web
client, website and webservice API. Because COffice does not control the creation
of workers anymore it can not setup cron or livechat workers

Cron Workers
------------

To run cron jobs for an COffice deployment as a WSGI application requires

* A classical COffice (run via ``coffice-bin``)
* Connected to the database in which cron jobs have to be run (via
  :option:`coffice-bin -d`)
* Which should not be exposed to the network. To ensure cron runners are not
  ...
  entirely with :option:`coffice-bin --no-http` or setting ``http_enable = False``
...
* Deploy a threaded version of COffice (instread of a process-based preforking
  one) and redirect only requests to URLs starting with ``/longpolling/`` to
  that COffice, this is the simplest and the longpolling URL can double up as
  the cron instance.
* Deploy an evented COffice via ``coffice-gevent`` and proxy requests starting
  with ``/longpolling/`` to
  :option:`the longpolling port <coffice-bin --longpolling-port>`.

Serving Static Files
====================

For development convenience, COffice directly serves all static files in its
...
COffice static files live in each module's ``static/`` folder, so static files
...
- Use appropriate database filters ( :option:`--db-filter <coffice-bin --db-filter>`)
  to restrict the visibility of your databases according to the hostname.
  See :ref:`db_filter`.
  You may also use :option:`-d <coffice-bin -d>` to provide your own (comma-separated)
...
- Once your ``db_name`` and ``db_filter`` are configured and only match a single database
  ...
  (this is also exposed as the :option:`--no-database-list <coffice-bin --no-database-list>`
  command-line option)

- Make sure the PostgreSQL user (:option:`--db_user <coffice-bin --db_user>`) is *not* a super-user,
  ...
  See also :ref:`setup/deploy/coffice`.
...
- Keep installations updated by regularly installing the latest builds,
  ...
  https://www.coffice.com/page/download or http://nightly.coffice.com
...
- Run COffice behind a web server providing HTTPS termination with a valid SSL certificate,
  ...
  and then enable the :option:`proxy mode <coffice-bin --proxy-mode>` option.
...
threat should not be neglected for COffice servers. COffice emits a log entry whenever a login attempt
...
Failed login::
      ...
      2018-07-05 14:56:31,506 24849 INFO db_name coffice.addons.base.res.res_users: Login failed for db:db_name login:admin from 127.0.0.1

Successful login::

      2018-07-05 14:56:31,506 24849 INFO db_name coffice.addons.base.res.res_users: Login successful for db:db_name login:admin from 127.0.0.1
...
10 failed login attempts are detected from the same IP within 1 minute::
    ...
    logpath = /var/log/coffice.log  ;  set the actual coffice log path here
...
:ref:`setup/deploy/coffice` mentioned ``admin_passwd`` in passing.
...
COffice is supported by multiple browsers for each of its versions. No
distinction is made according to the browser version in order to be
up-to-date. COffice is supported on the current browser version. We support
...
.. note::
    ...
    Since COffice 13.0, ES6 is supported.  Therefore, IE support is dropped.


.. [#different-machines]
    to have multiple COffice installations use the same PostgreSQL database,
...
.. _POSBox: https://www.coffice.com/page/point-of-sale-hardware#part_2
