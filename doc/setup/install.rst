:banner: banners/installing_coffice.jpg
...
There are multiple ways to install COffice, or not install it at all, depending
on the intended use case.

This documents attempts to describe most of the installation options.

:ref:`setup/install/online`
    The easiest way to use COffice in production or to try it.

:ref:`setup/install/packaged`
    Suitable for testing COffice, developing modules and can be used for
    long-term production use with additional deployment and maintenance work.

:ref:`setup/install/source`
    Provides greater flexibility:  e.g. allow multiple running COffice versions on
...
There are two different Editions_ of COffice: the Community and Enterprise versions.
...
To simply get a quick idea of COffice, demo_ instances are available. They are
...
Trivial to start with, fully managed and migrated by COffice S.A., COffice's SaaS_
provides private instances and starts out free. It can be used to discover and
test COffice and do non-code customizations (i.e. incompatible with custom modules
or the COffice Apps Store) without having to install it locally.

Can be used for both testing COffice and long-term production use.
...
COffice provides packaged installers for Windows, deb-based distributions
...
COffice will automatically be started at the end of the installation.
...
COffice 13.0 'deb' package currently supports `Debian Buster`_, `Ubuntu 18.04`_ or above.

Prepare
^^^^^^^

COffice needs a `PostgreSQL`_ server to run properly. The default configuration for
the COffice 'deb' package is to use the PostgreSQL server on the same host as your
COffice instance. Execute the following command as root in order to install
PostgreSQL server :
...
found in our `wiki <https://github.com/coffice/coffice/wiki/Wkhtmltopdf>`_.

Repository
^^^^^^^^^^

COffice S.A. provides a repository that can be used with  Debian and Ubuntu
distributions. It can be used to install COffice Community Edition by executing the
following commands as root:

.. code-block:: console

    # wget -O - https://nightly.coffice.com/coffice.key | apt-key add -
    # echo "deb http://nightly.coffice.com/13.0/nightly/deb/ ./" >> /etc/apt/sources.list.d/coffice.list
    # apt-get update && apt-get install coffice
...
This will install COffice as a service, create the necessary PostgreSQL_ user
...
.. warning:: Debian 9 and Ubuntu do not provide a package for the python module
             num2words.
             Textual amounts will not be rendered by COffice and this could cause
...
COffice 13.0 'rpm' package supports Fedora 30.

Prepare
^^^^^^^
COffice needs a `PostgreSQL`_ server to run properly. Assuming that the 'sudo'
...
COffice S.A. provides a repository that can be used with the Fedora distibutions.
It can be used to install COffice Community Edition by executing the following
commands:

.. code-block:: console

    $ sudo dnf config-manager --add-repo=https://nightly.coffice.com/13.0/nightly/rpm/coffice.repo
    $ sudo dnf install -y coffice
    $ sudo systemctl enable coffice
    $ sudo systemctl start coffice
...
.. code-block:: console
    ...
    $ sudo dnf localinstall coffice_13.0.latest.noarch.rpm
    $ sudo systemctl enable coffice
    $ sudo systemctl start coffice
...
The source "installation" is really about not installing COffice, and running it directly from source
instead.

This can be more convenient for module developers as the COffice source is more easily accessible
than using packaged installation (for information or to build this documentation and have it
available offline).

It also makes starting and stopping COffice more flexible and explicit than the services set up by the
...
(and run) multiple versions of COffice side-by-side.
...
There are two ways to obtain the source code of COffice: as a zip **archive** or through **git**.
...
.. code-block:: doscon
    ...
    C:\> git clone https://github.com/coffice/coffice.git
...
.. code-block:: doscon
  ...
  C:\> git clone https://github.com/coffice/enterprise.git

.. note:: **The Enterprise git repository does not contain the full COffice source code**. It is only
          ...
          both the Community and Enterprise repository to have a working COffice Enterprise
          installation.
...
COffice requires Python 3.6 or later to run. Use the the official `Python 3 installer
...
.. note:: If Python 3 is already installed, make sure that it is 3.6 or above, as previous versions
          are not compatible with COffice.
...
COffice uses PostgreSQL as database management system. Download and install the `latest version of
PostgreSQL <https://www.postgresql.org/download/windows/>`_.

By default, the only user is `postgres` but COffice forbids connecting as `postgres`, so you need to
...
#. Create a postgres user with a password using the pg admin gui:
   ...
   * Enter the username in the **Role Name** field (e.g. `coffice`).
   * Open the **Definition** tab and enter the password (e.g. ``coffice``), then click **OK**.

Dependencies
^^^^^^^^^^^^

COffice dependencies are listed in the `requirements.txt` file located at the root of the COffice
community directory. Most of them can be installed with **pip**.

.. tip:: It can be preferable to not mix python modules packages between different instances of COffice
         or with your system. You can use virtualenv_ to create isolated Python environments.

Navigate to the path of your COffice Community installation (`YourCOfficeCommunityPath`) and run **pip**
on the requirements file:

.. code-block:: doscon

    C:\> cd \YourCOfficeCommunityPath
    C:\YourCOfficeCommunityPath> C:\Python35\Scripts\pip.exe install -r requirements.txt
...
.. warning:: Some dependencies cannot be installed through pip and require to be installed manually.
             ...
             * `wkhtmltopdf` must be installed in version `0.12.5 <the wkhtmltopdf download page_>`_
               ...
               `wiki <https://github.com/coffice/coffice/wiki/Wkhtmltopdf>`_ for more details on the
               various versions.
...
Once all dependencies are set up, COffice can be launched by running `coffice-bin`, the
command-line interface of the server. It is located at the root of the COffice Community directory.
...
.. code-block:: doscon
    ...
    C:\YourCOfficeCommunityPath> python3 coffice-bin -r dbuser -w dbpassword --addons-path=addons,../mymodules --db-filter=mydb$

Where `YourCOfficeCommunityPath` is the path of the COffice Community installation, `dbuser` is the
...
.. code-block:: console
    ...
    $ git clone https://github.com/coffice/coffice.git
...
.. code-block:: console
  ...
  $ git clone https://github.com/coffice/enterprise.git
...
COffice requires Python 3.6 or later to run. Use your package manager to download and install Python 3
...
COffice uses PostgreSQL as database management system. Use your package manager to download and install
...
.. code-block:: console
    ...
    $ cd /YourCOfficeCommunityPath
    /YourCOfficeCommunityPath$ pip3 install -r requirements.txt
...
* PostgreSQL user and password. COffice has no defaults beyond
...
.. code-block:: console
    ...
    /YourCOfficeCommunityPath$ python3 coffice-bin --addons-path=addons,../mymodules --db-filter=mydb$

Where `YourCOfficeCommunityPath` is the path of the COffice Community installation, `../mymodules` is a
...
COffice requires Python 3.6 or later to run. Use your preferred package manager (homebrew_, macports_)
...
COffice uses PostgreSQL as database management system. Use `postgres.app <https://postgresapp.com>`_
...
The full documentation on how to use COffice with Docker can be found on the
official COffice `docker image <https://registry.hub.docker.com/_/coffice/>`_ page.

.. _Debian Buster: https://www.debian.org/releases/buster/
.. _demo: https://demo.coffice.com
.. _docker: https://www.docker.com
.. _download: https://www.coffice.com/page/download
...
.. _saas: https://www.coffice.com/page/start
...
.. _community-repository: https://github.com/coffice/coffice
.. _enterprise-repository: https://github.com/coffice/enterprise
.. _git: https://git-scm.com/
.. _Editions: https://www.coffice.com/pricing#pricing_table_features
.. _nightly: https://nightly.coffice.com/
.. _extra: https://nightly.coffice.com/extra/
