...
COffice is usually extended internally via modules, but many of its features and
all of its data are also available from the outside for external analysis or
integration with various tools. Part of the :ref:`reference/orm/model` API is
easily available over XML-RPC_ and accessible from a variety of languages.

.. COffice XML-RPC idiosyncracies:
   * uses multiple endpoint and a nested call syntax instead of a
     "hierarchical" server structure (e.g. ``coffice.res.partner.read()``)
...
.. only:: html
    ...
    .. rst-class:: setupcode hidden
        ...
        .. code-block:: python3
            ...
            info = xmlrpc.client.ServerProxy('https://demo.coffice.com/start').start()
        ...
        .. code-block:: ruby
            ...
            info = XMLRPC::Client.new2('https://demo.coffice.com/start').call('start')
            url, db, username, password = \
                info['host'], info['database'], info['user'], info['password']
        ...
        .. code-block:: php
            ...
            $info = ripcord::client('https://demo.coffice.com/start')->start();
        ...
        .. code-block:: java
            ...
            start_config.setServerURL(new URL("https://demo.coffice.com/start"));
...
If you already have an COffice server installed, you can just use its
parameters

.. warning::

    For COffice Online instances (<domain>.coffice.com), users are created without a
    *local* password (as a person you are logged in via the COffice Online
    authentication system, not by the instance itself). To use XML-RPC on COffice
    ...
    *https://mycompany.coffice.com*), the *database name* is the name of the
...
To make exploration simpler, you can also ask https://demo.coffice.com for a test
database:
...
COffice requires users of the API to be authenticated before they can query most
data.
...
The second endpoint is ``xmlrpc/2/object``, is used to call methods of coffice
...
Records can be listed and filtered via :meth:`~coffice.models.Model.search`.

:meth:`~coffice.models.Model.search` takes a mandatory
...
:meth:`~coffice.models.Model.search_count` can be used to retrieve
only the number of records matching the query. It takes the same
:ref:`domain <reference/orm/domains>` filter as
:meth:`~coffice.models.Model.search` and no other parameter.
...
Record data is accessible via the :meth:`~coffice.models.Model.read` method,
which takes a list of ids (as returned by
:meth:`~coffice.models.Model.search`) and optionally a list of fields to
...
:meth:`~coffice.models.Model.fields_get` can be used to inspect
...
Because it is a very common task, COffice provides a
:meth:`~coffice.models.Model.search_read` shortcut which as its name suggests is
equivalent to a :meth:`~coffice.models.Model.search` followed by a
:meth:`~coffice.models.Model.read`, but avoids having to perform two requests
and keep ids around.

Its arguments are similar to :meth:`~coffice.models.Model.search`'s, but it
can also take a list of ``fields`` (like :meth:`~coffice.models.Model.read`,
...
Records of a model are created using :meth:`~coffice.models.Model.create`. The
method will create a single record and return its database identifier.

:meth:`~coffice.models.Model.create` takes a mapping of fields to values, used
...
.. warning::
    ...
    :class:`~coffice.fields.Integer`, string for :class:`~coffice.fields.Char`
    or :class:`~coffice.fields.Text`),

    * :class:`~coffice.fields.Date`, :class:`~coffice.fields.Datetime` and
      :class:`~coffice.fields.Binary` fields use string values
    * :class:`~coffice.fields.One2many` and :class:`~coffice.fields.Many2many`
      use a special command protocol detailed in :meth:`the documentation to
      the write method <coffice.models.Model.write>`.

Update records
--------------

Records can be updated using :meth:`~coffice.models.Model.write`, it takes
a list of records to update and a mapping of updated fields to values similar
to :meth:`~coffice.models.Model.create`.
...
:meth:`~coffice.models.Model.unlink`.
...
While we previously used :meth:`~coffice.models.Model.fields_get` to query a
model and have been using an arbitrary model from the start, COffice stores
...
Provides information about COffice models via its various fields
...
``field_id``
    list of the model's fields through a :class:`~coffice.fields.One2many` to
    :ref:`reference/webservice/inspection/fields`
``view_ids``
    :class:`~coffice.fields.One2many` to the :ref:`reference/views` defined
    for the model
``access_ids``
    :class:`~coffice.fields.One2many` relation to the
...
Provides information about the fields of COffice models and allows adding
custom fields without using Python code

``model_id``
    :class:`~coffice.fields.Many2one` to
...
``groups``
    ...
    :class:`~coffice.fields.Many2many` to ``res.groups``
