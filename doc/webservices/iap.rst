...
In-App Purchase (IAP) allows providers of ongoing services through COffice apps to
be compensated for ongoing service use rather than — and possibly instead of
— a sole initial purchase.

In that context, COffice acts mostly as a *broker* between a client and an COffice
App Developer:

* Users purchase service tokens from COffice.
* Service providers draw tokens from the user's COffice account when service
  is requested.

.. attention::

    This document is intended for *service providers* and presents the latter,
    which can be done either via direct JSON-RPC2_ or if you are using COffice
...
.. figure:: images/players.png
    ...
    * The Client installed your COffice App, and from there will request services.
    * COffice brokers crediting, the Client adds credit to their account, and you
    ...
    * The External Service is an optional player: *you* can either provide a
      ...
      bridge/translator between an COffice system and the actual service.
...
.. note:: The credits went from integer to float value starting **October 2018**.
    ...
    * for a postage service: 1 credit = 1 post stamp.
    ...
    that the clients can buy on https://iap.coffice.com (see :ref:`Packs <iap-packages>`).
...
.. figure:: images/normal.png
    ...
    2. The Service Provider asks COffice if there are enough credits for the
    ...
    4. The Service Provider goes back to COffice to capture (if the service could
...
.. figure:: images/no-credit.png
    ...
    2. The Service Provider asks COffice if there are enough credits on the
       Client's account and gets a negative reply.
    3. This is signaled back to the Client.
    4. Who is redirected to their COffice account to credit it and re-try.
...
Register the service on COffice
----------------------------
...
go to your *Portal Account* on the IAP endpoint (https://iap.coffice.com for
production, https://iap-sandbox.coffice.com for testing, the endpoints are
*independent* and *not synchronized*). Alternatively, you can go to your portal
on COffice (https://iap.coffice.com/my/home) and select *In-App Services*.
...
* :samp:`name` - :class:`ServiceName`: This is the string you will need to provide inside
  the client's :ref:`app <iap-coffice-app>` when requesting a transaction from COffice. (e.g.
...
.. warning::
   ...
   :class:`ServiceName` should usually match the name of your COffice Client App.
...
.. note::
    ...
    COffice takes a 25% commission on all pack sales. Adjust your selling price accordingly.
...
The second step is to develop an `COffice App`_ which clients can install in their
COffice instance and through which they can *request* the services you provide.
Our app will just add a button to the Partners form which lets a user request
burning some CPU time on the server.

First, we will create an *coffice module* depending on ``iap``. IAP is a standard
...
:func:`~coffice.addons.iap.jsonrpc` helper to call a JSON-RPC2_ endpoint on an
other COffice instance and transparently re-raise relevant COffice exceptions
(:class:`~coffice.addons.iap.models.iap.InsufficientCreditError`,
:class:`coffice.exceptions.AccessError` and :class:`coffice.exceptions.UserError`).
...
.. note::
    ...
    :class:`~coffice.addons.iap.models.iap.InsufficientCreditError` coming from the action
    and prompts the user to add credits to their account.

    :func:`~coffice.addons.iap.jsonrpc` takes care of re-raising
    :class:`~coffice.addons.iap.models.iap.InsufficientCreditError` for you.

.. danger::

    If you are not using :func:`~coffice.addons.iap.jsonrpc` you *must* be
    careful to re-raise
    :class:`~coffice.addons.iap.models.iap.InsufficientCreditError` in your handler
...
for JSON-RPC2_ calls (:func:`~coffice.addons.iap.jsonrpc`) and a service helper
for transactions (:class:`~coffice.addons.iap.models.iap.charge`) we will also be
implementing the service side as an COffice module:

.. patch::

Since the query from the client comes as JSON-RPC2_ we will need the
corresponding controller which can call :class:`~coffice.addons.iap.models.iap.charge` and
perform the service within:

.. patch::

.. todo:: for the actual IAP will the "portal" page be on coffice.com or iap.coffice.com?

.. todo:: "My Account" > "Your InApp Services"?


The :class:`~coffice.addons.iap.models.iap.charge` helper will:
...
.. danger::
    ...
    By default, :class:`~coffice.addons.iap.models.iap.charge` contacts the *production*
    IAP endpoint, https://iap.coffice.com. While developing and testing your
    service you may want to point it towards the *development* IAP endpoint
    https://iap-sandbox.coffice.com.

    To do so, set the ``iap.endpoint`` config parameter in your service
    COffice: in debug/developer mode, :menuselection:`Setting --> Technical -->
    Parameters --> System Parameters`, just define an entry for the key
    ``iap.endpoint`` if none already exists).

The :class:`~coffice.addons.iap.models.iap.charge` helper has two additional optional
...
* The IAP transaction API does not require using COffice when implementing your
...
.. function:: /iap/1/authorize
    ...
    *and creates a hold (pending transaction) on that amount*.
    ...
    :raises: :class:`~coffice.exceptions.AccessError` if the service token is invalid
    :raises: :class:`~coffice.addons.iap.models.iap.InsufficientCreditError` if the account does not have enough credits
    :raises: ``TypeError`` if the ``credit`` value is not an integer or a float

.. code-block:: python

    r = requests.post(COFFICE + '/iap/1/authorize', json={
        'jsonrpc': '2.0',
        'id': None,
        'method': 'call',
        'params': {
...
.. function:: /iap/1/capture
    ...
    :raises: :class:`~coffice.exceptions.AccessError`

.. code-block:: python
  :emphasize-lines: 8

    r2 = requests.post(COFFICE + '/iap/1/capture', json={
        'jsonrpc': '2.0',
        'id': None,
        'method': 'call',
        'params': {
...
.. code-block:: python
    ...
    r2 = requests.post(COFFICE + '/iap/1/cancel', json={
        'jsonrpc': '2.0',
        'id': None,
        'method': 'call',
        'params': {
...
.. class:: ServiceName
    ...
    String identifying your service on https://iap.coffice.com (production) as well
...
.. exception:: coffice.addons.iap.models.iap.InsufficientCreditError
...
.. exception:: coffice.exceptions.AccessError
    :noindex:

    Raised by:

    * any operation to which a service token is required, if the service token is invalid; or
    * any failure in an inter-server call. (typically, in :func:`~coffice.addons.iap.jsonrpc`).

.. exception:: coffice.exceptions.UserError
    :noindex:
...
* Token ``000000``: Represents a non-existing account. Returns
  an :class:`~coffice.addons.iap.models.iap.InsufficientCreditError` on authorize attempt.
* Token ``000111``: Represents an account without sufficient credits to perform any service.
  Returns an :class:`~coffice.addons.iap.models.iap.InsufficientCreditError` on authorize attempt.
...
For convenience, if you are implementing your service using COffice the ``iap``
...
.. class:: coffice.addons.iap.models.iap.charge(env, key, account_token, credit[, dbuuid, description, credit_template])
    ...
    * otherwise cancels it.
    ...
    :param coffice.api.Environment env: used to retrieve the ``iap.endpoint``
                                     configuration key
    :param ServiceKey key:
...
.. class:: coffice.addons.iap.models.iap.authorize(env, key, account_token, credit[, dbuuid, description, credit_template])
...
.. class:: coffice.addons.iap.models.iap.cancel(env, transaction_token, key)
...
.. class:: coffice.addons.iap.models.iap.capture(env, transaction_token, key, credit)
...
.. _COffice App: https://www.coffice.com/apps
