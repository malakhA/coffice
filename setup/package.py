...
exec(open(os.path.join(ROOTDIR, 'coffice', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/coffice && \
    groupadd -g %(group_id)s coffice && \
    useradd -u %(user_id)s -g coffice coffice -d /var/lib/coffice && \
    mkdir /data && \
    chown coffice:coffice /var/lib/coffice /data
USER coffice
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/coffice_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False):
    ...
    run_cmd(cmd + ['%s/' % args.coffice_dir, args.build_dir])
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'coffice/addons'))
...
#  Docker stuffs
class COfficeTestTimeoutError(Exception):
    pass


class COfficeTestError(Exception):
    pass
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'coffice-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='coffice', exposed_port=None, detach=False, timeout=None):
    ...
    def test_coffice(self):
        logging.info('Starting to test COffice install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'coffice.pid')):
                _rpc_count_modules(port=self.exposed_port)
                self.stop()
                return
        if self.is_running():
            self.stop()
            raise COfficeTestTimeoutError('COffice pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise COfficeTestError('Error while installing/starting COffice after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'coffice-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/coffice-*.tar.gz' % self.args.build_dir)[0], '%s/coffice_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/coffice-*.zip' % self.args.build_dir)[0], '%s/coffice_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'pip3 install /data/src/coffice_%s.%s.tar.gz' % (VERSION, TSTAMP),
            'su postgres -s /bin/bash -c "createuser -s coffice"',
            'su postgres -s /bin/bash -c "createdb mycompany"',
            'su coffice -s /bin/bash -c "coffice -d mycompany -i base --stop-after-init"',
            'su coffice -s /bin/bash -c "coffice -d mycompany --pidfile=/data/src/coffice.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'coffice-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_coffice()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/coffice (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../coffice_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'coffice-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            '/usr/bin/dpkg -i /data/src/coffice_%s.%s_all.deb ; /usr/bin/apt-get install -f -y' % (VERSION, TSTAMP),
            'su coffice -s /bin/bash -c "coffice -d mycompany -i base --stop-after-init"',
            'su coffice -s /bin/bash -c "coffice -d mycompany --pidfile=/data/src/coffice.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'coffice-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_coffice()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py --quiet bdist_rpm', self.args.build_dir, 'coffice-rpm-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/coffice-*.noarch.rpm' % self.args.build_dir)[0], '%s/coffice_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'dnf install -d 0 -e 0 /data/src/coffice_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su coffice -s /bin/bash -c "coffice -c /etc/coffice/coffice.conf -d mycompany -i base --stop-after-init"',
            'su coffice -s /bin/bash -c "coffice -c /etc/coffice/coffice.conf -d mycompany --pidfile=/data/src/coffice.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'coffice-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_coffice()
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/cofficebuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing COffice files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"COffice %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"COffice {sv}"/python/python.exe \'c:\\Program Files\\COffice {sv}\\server\\coffice-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def build_exe(args):
    ...
    shutil.copy(glob('%s/openerp*.exe' % args.build_dir)[0], '%s/coffice_%s.%s.exe' % (args.build_dir, VERSION, TSTAMP))
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/coffice/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/coffice/vm/win1036/id_rsa', help="%(default)s")
    ...
    parsed_args.coffice_dir = ROOTDIR
    return parsed_args
